...
type ExitError struct {
	// Err holds only a normalized exit classification: raw command/process errors
	// must not cross the package boundary, because their text carries the command
	// line (and with it the token the process was started with).
	Err error
}
