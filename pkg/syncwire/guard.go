...
// GuardPayload 是同步载荷的结构守卫。kind 让唯一可携带 API Key 的
// llm_provider 与所有其它对象明确分开；调用者不可省略它。它按**键名**挡住三类东西：
//
//   - 本地自增 ID（键名以 id 结尾、取值是数字）。跨机引用一律用同步标识、agentred
//     指纹或 provider_key 表达，它们全是字符串；这个形状只可能是某台机器的本地主键，
//     在别的机器上指向完全不同的对象。
...
// 按设计随 backend 上行，在账号下明文存放。里面不会有 App 自管的那 15 个保留键
// （ANTHROPIC_API_KEY / OPENAI_API_KEY 等，由桌面端的 agent_backend_entity.Check
// 拒绝入库），但用户自己往里放的任何别的密钥都会原样过机。
//
// 这条守卫不是凭据扫描器，别把它当成一个——要挡住 env_json 就得整个字段不上行，
// 那是规格决定，不是守卫能顺手做掉的事。
//
// **两个宿主跑的是这一份实现,不是两份对齐的抄本。** 桌面端在上行前调用它,让坏载荷
// 根本发不出去;服务端在落库前调用它,保护账号里的其它设备。从前它在两个仓库各有一份
// 逐条同规则的代码,靠注释与两份相同的测试向量人肉对齐 —— 那个理由早就不成立了:守卫
// 零外部依赖,而这个 module 正是为「服务端也能 import」而存在的。
//
// 规则一旦分叉,后果是单条被拒(server 回 rejected,本端出队并进 R5 列表),不会堵住
// 整条队列 —— 这也是当初两份抄本能长期活下来的原因。
//
// 空载荷合法：墓碑不带正文。
