...
// 守卫的签名是契约的一部分:kind 不可省略 —— 唯一可携带 API Key 的 llm_provider
// 靠它与所有其它对象分开。
...
// TestGuardPayload_GivenProviderKind_AllowsAPIKey kind 是守卫唯一的分档依据：
// provider 自己的 API Key 随账号同步，同一个键在别的任何 kind 上都是禁的；
// agent_backend 的 cli_path 同理，那是本机路径。
...
// TestGuardPayload_GivenTheBoardPayloads_PassesEveryOne 看板并入同步组后新增的
// 三种载荷（规格「数据与迁移 › 同步（跨仓）」的载荷形状表）逐个过守卫：跨机引用
// 一律是 *_sync_id 字符串，没有一个键以 id 结尾配数字值。
func TestGuardPayload_GivenTheBoardPayloads_PassesEveryOne(t *testing.T) {
	for kind, payload := range map[string]string{
		syncwire.KindLabel: `{"name":"bug","tone":"red","status":1}`,
		syncwire.KindIssue: `{"title":"t","description":"d","stage":"doing","position":1.5,` +
			`"project_sync_id":"p","agent_sync_id":"a","agent_backend_sync_id":"b",` +
			`"llm_provider_key":"anthropic-main","llm_model_key":"opus","closed_at":0}`,
		syncwire.KindIssueLabel: `{"issue_sync_id":"i","label_sync_id":"l"}`,
	} {
		assert.NoError(t, syncwire.GuardPayload(kind, []byte(payload)), kind)
	}
}

// TestGuardPayload_GivenABoardPayloadCarryingALocalID_Rejects 反向断言：任务载荷
// 里塞一个本机自增项目 ID 必须被拒——它在别的机器上指向完全不同的项目。
func TestGuardPayload_GivenABoardPayloadCarryingALocalID_Rejects(t *testing.T) {
	for _, payload := range []string{
		`{"title":"t","project_id":42}`,
		`{"title":"t","assignee_agent_id":42}`,
		`{"issue_id":1,"label_id":2}`,
	} {
		assert.ErrorIs(t, syncwire.GuardPayload(syncwire.KindIssue, []byte(payload)),
			syncwire.ErrPayloadLocalID, payload)
	}
}
