...
//   - content_block_start:模型开始产出一个输出块 —— 翻成 EventContentBlockStart,
//     上层拿它记首 token。工具入参那一跳没有可见增量,只有这条能证明模型已开口。
...
//     (它的计时由上面的 content_block_start 承担)。
...
func parseStreamEventFrame(f rawFrame, sid string, p *partialText, remember func(*rawUsage)) []Event {
	...
	case "content_block_start":
		// 纯计时信号(见 EventContentBlockStart 注释):不带内容,不进 accumulator,
		// 也不参与 partialText 去重账本 —— 它和 merged assistant 帧产出的块不是
		// 同一种东西,不存在"吐两遍"的问题。
		return []Event{{Kind: EventContentBlockStart, SessionID: sid}}
