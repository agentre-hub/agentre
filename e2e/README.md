...
`lib/target.mjs` derives one checkout-scoped data directory, file-keychain directory, browser directory, session file, bridge port, and CDP port. It rejects the installed app root, the development root, arbitrary directories, non-loopback origins, and the ordinary development bridge. A second worktree derives a different target. The launcher never adopts an unrecorded process holding its port: it drives only its own recorded session, and reclaims the port only when every holder is provably this checkout's orphan — this checkout's path on the command line and a working directory inside the checkout — which is the state a killed `wails dev` leaves behind, because the app binary it launched keeps the port while its parent is gone. Anything it cannot prove is refused without a signal being sent, so `make verify-up` no longer needs a `verify-down` inserted between a crash and the restart being verified. Leftover `vite` dev servers are reaped one process at a time under the same ownership check instead of by pattern, so a `vitest` running in this checkout — `vite` is a substring of `vitest` — is never killed along with them.
