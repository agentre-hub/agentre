...
func newRunCmdWithDeps(deps runDeps) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if err := requireServerMatchesLogin(st, config.serverURL); err != nil {
				return err
			}
...
//
// 闸门与 login 那条路同源（已登录必须先 logout），措辞也保持一致：换 server 是
// 一次重新登录，不是改一个配置项。
//
// 只在两边都有地址且**确实不同**时拒：登录时没记下地址的旧状态照常放行，让这一次
// run 把地址补上。
func requireServerMatchesLogin(st *state.State, serverURL string) error {
	if !st.IsLoggedIn() {
		return nil
	}
	...
	return newUsageError(
		"daemon is logged into %s; run agentred logout before pointing it at %s",
