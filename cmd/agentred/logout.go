...
func newLogoutCmd() *cobra.Command {
	return newLogoutCmdWithDeps(logoutDeps{
...
func newLogoutCmdWithDeps(deps logoutDeps) *cobra.Command {
	return &cobra.Command{
		Use:   "logout",
		Short: "Log this daemon out of its account",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			// 先尽力通知账号侧解除授权，再清本地——顺序不可换，Logout 会把凭据一并抹掉。
			//
			...
			if err := st.Save(); err != nil {
				return fmt.Errorf("save logged-out state: %w", err)
			}
			_, _ = fmt.Fprintln(cmd.OutOrStdout(), "Logged out. This daemon no longer belongs to the account.")
			return nil
		},
	}
}
...
func revokeAccountAuthorization(
	...
	if client == nil || serverURL == "" {
		// 从没登录过账号（只做过 LAN 配对）：没有账号侧可通知，logout 仍是纯本地操作。
		return nil
	}
	token := credential.AccessToken
	if credential.RefreshToken != "" {
		// access token 是分钟级的，而 logout 的前置条件正是「daemon 已经停了」——
...
// 换来的这张只服务紧接着的那一次撤销，而 state.json 下一步就要被 Logout 清空。
