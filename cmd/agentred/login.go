...
func newLoginCmdWithDeps(deps loginDeps) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "login",
		Short: "Log this daemon into an account through device authorization",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if st.IsLoggedIn() {
				return fmt.Errorf("daemon is already logged in; run agentred logout before logging into another account")
			}
...
func login(cmd *cobra.Command, deps loginDeps, st *state.State, serverURL string) error {
	...
	st.Mutate(func(current *state.State) {
		current.AccountServerURL = serverURL
	})
	if err := st.Save(); err != nil {
		return fmt.Errorf("save account login: %w", err)
	}
	...
	_, _ = fmt.Fprintln(out, "Logged in. This daemon now belongs to the account.")
	return nil
}
