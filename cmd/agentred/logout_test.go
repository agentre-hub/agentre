...
// 几秒钟的假象。它尤其致命 —— 用户通常是先 logout 再 login,第一步就被回滚的话,第二步
// 会以「已归属」为由拒绝,或者登录成功却同样被覆盖。
func TestLogoutRefusesWhileDaemonIsRunning(t *testing.T) {
	dir := t.TempDir()
	...
	cmd := newLogoutCmdWithDeps(logoutDeps{
	...
	assert.Equal(t, 0, networkCalls, "拒绝掉的 logout 不该顺手撤销一台仍在服务的设备")

	got, loadErr := state.Load(dir)
	require.NoError(t, loadErr)
	assert.True(t, got.IsLoggedIn(), "a refused logout must not report success it cannot keep")
}

// 从未登录过账号的 daemon 上 logout 仍然是纯本地操作（前置规格 R19）：没有服务端
// 地址、也没有凭据，无处可通知，因此一个请求都不该发出去。
func TestLogoutClearsTheAccountWhenNoDaemonIsRunning(t *testing.T) {
	dir := t.TempDir()
	st, err := state.Load(dir)
	require.NoError(t, err)
	st.Mutate(func(s *state.State) { s.AccountID = "account-1" })
	...
	assert.Equal(t, 0, networkCalls, "没有服务端地址时 logout 仍是纯本地操作")
...
// 解除归属要先尽力通知账号侧「解除授权」，让控制台撤销与 `agentred logout` 走**同一条**
// 服务端路径（撤销 token 链、拉黑 jti、并把只属于这台机器的账号级同步对象落墓碑），
// 而不是两处各写一份慢慢漂移。
//
// 先换一张新的 access token 再调撤销：它是分钟级的（server AccessTTL=15m），而 logout
// 的前置条件正是「daemon 已经停了」——那份存盘的 access token 在真实场景里几乎总是过期
// 的，直接拿它去撤销等于这条通知永远打不出去。
func TestLogoutBestEffortRevokesTheAccountAuthorizationBeforeClearingLocalState(t *testing.T) {
	dir := t.TempDir()
	st, err := state.Load(dir)
	require.NoError(t, err)
...
// 用户执行了 logout 就必须解除归属：服务端不可达（机器搬走了、账号服务停了、
// 凭据早已被对面撤销）只记一行提示，绝不阻挡本地清理——否则一台再也连不上账号的
// 机器将永远无法回到未登录状态，也就永远无法重新配对或登录另一个账号。
func TestLogoutClearsLocalAccountStateEvenWhenTheServerIsUnreachable(t *testing.T) {
	dir := t.TempDir()
	st, err := state.Load(dir)
	require.NoError(t, err)
	st.Mutate(func(s *state.State) {
