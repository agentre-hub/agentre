...
import sessionSummaryMinimalFixture from "../../fixtures/session-summary-minimal.json";
...
describe("wire 编解码:与 Go 侧黄金样本逐字段同构", () => {
  ...
  it("SessionSummary 解出 R7 新列;老会话如实留空(键缺失)", () => {
    ...
    const minimal = assertRoundTrip(
      decodeSessionSummary,
      (v) => JSON.stringify(v),
      sessionSummaryMinimalFixture,
      "SessionSummary(minimal)",
    );
    expect(minimal.title).toBeUndefined();
    expect(minimal.agentSyncId).toBeUndefined();
    expect(minimal.providerSessionId).toBeUndefined();
    expect(minimal.lifecycleState).toBe("idle");
    expect(minimal.peerFingerprint).toBe("fp-desktop");
    expect(minimal.lastMessageAt).toBeUndefined();
    // 还没跑过第一轮的会话不带这一格:键缺席 = 跟随后端配置,不是「用户选了默认档」。
