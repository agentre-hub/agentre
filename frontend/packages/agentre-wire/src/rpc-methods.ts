...
export const rpcMethods = {
  ...
  sessionCounts: method(
    "sessionCounts",
    59,
    pb.SessionCountsRequestSchema,
    pb.SessionCountsResponseSchema,
  ),
  ...
  // agentred 自更新(spec 2026-09-03):应答只回受理结果,受理之后连接必然断开
  // (daemon 重启),升级结果由调用方从重连后读到的版本号推断。
  agentredSelfUpdate: method(
    "agentredSelfUpdate",
    58,
    pb.AgentredSelfUpdateRequestSchema,
    pb.AgentredSelfUpdateResponseSchema,
  ),
} as const;
