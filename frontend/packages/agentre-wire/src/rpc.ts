...
/**
 * 客户端要的那一轮开始了。自主续轮那一路一直有开始通知，客户端发起的这一路没有，
 * 而不是自己发起这一轮的订阅者（账号镜像、第二台桌面端、手机）因此只看得到轮次
 * 结束 —— 整轮里都把这条对话显示成闲着。
 */
export interface TurnStartedNotificationFrame {
...
function encodeNotification(value: RpcNotificationBody) {
  ...
  if (value.case === "turnStartedNotification") {
    return create(RpcNotificationSchema, {
      payload: {
        case: "turnStarted",
        value: create(TurnStartedNotificationSchema, {
          conversationId: value.conversationId,
          seq: BigInt(value.seq),
        }),
      },
...
function decodeNotification(
  ...
  if (value.payload.case === "turnStarted") {
    const v = value.payload.value;
    return {
      case: "turnStartedNotification",
      conversationId: v.conversationId,
...
export const ProtobufRpcCodec = {
  ...
  encode(frame: ProtobufRpcFrame): Uint8Array {
    if (
      ...
      frame.body.case === "turnStartedNotification" ||
