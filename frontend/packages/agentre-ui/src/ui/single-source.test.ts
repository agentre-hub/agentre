...
/**
 * 曾经在 `src/engine/ui/` 下有过第二份实现的全部原语。
 *
 * 名单必须覆盖那个目录的**每一个**原有条目（含当时已经退化成 re-export 壳的
 * badge / button / input / popover）——目录清空之后，任何一项都不许再回来，而
 * 「壳」和「下一份副本」在守卫眼里长得一样，所以壳也一并退场。
 */
const MERGED = [
  "alert",
  "badge",
...
describe("引擎私拷只剩一份", () => {
  it("engine/ui/ 这个目录本身已经不存在", () => {
    // 逐个文件断言拦不住「换个名字再来一份」：目录在，下一份私拷就有地方落脚。
    expect(existsSync(join(packageRoot, "src/engine/ui"))).toBe(false);
  });
