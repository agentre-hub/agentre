...
// 沙箱逃生口）。此处展示提示 + 一键补上这个键。
//
// `canAddIsSandbox` 问的是「宿主能不能**补这一个键**」，不是「能不能编辑整张 env
// 表」——两者从前是同一颗开关（`canEditEnvJSON`），于是浏览器宿主连这条提示都看不到。
// 它们必须分开：env_json 不下发浏览器（agentre-server 的 R19），整表编辑器在那一侧
// 永远开不了（拿不到现有的表，保存就会把用户自填的键连同密钥一起抹掉），但补一个
// 固定键是它做得到的——由服务端合并，见宿主的 addIsSandbox port。
export function DefaultPermissionModeField({
  ...
  onChange: (v: string) => void;
  isRemote: boolean;
  canAddIsSandbox: boolean;
  hasIsSandbox: boolean;
  onAddIsSandbox: () => void;
  // 补键失败时的原文。走服务端合并的宿主才可能失败（桌面端改的是本地 entries，
  // 不会失败），就地显示在提示块里——这个动作没有别的落点，弹窗没有统一的错误面。
  addIsSandboxError?: string;
}) {
  const { t } = useTranslation();
