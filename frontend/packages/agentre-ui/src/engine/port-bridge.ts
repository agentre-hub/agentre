...
export namespace llm_provider_svc {
  ...
  export type ModelInfo = { id: string; name?: string; vendor: string; contextWindow: number; maxOutput: number };
...
export async function LookupLLMModel(request: llm_provider_svc.LookupModelRequest) {
  const found = request.id === undefined
    ? undefined
...
export async function UpdateLLMProvider(request: llm_provider_svc.UpdateProviderRequest) { return { item: await ports().updateProvider(request.id, request as unknown as ProviderInput) }; }
...
export namespace httpgateway { export type GatewayStatus = GatewayStatusView; }
...
export async function ScanAndCreateAgentBackends() {
  if (ports().scanBackendResults) return { results: await ports().scanBackendResults!() };
  if (!ports().scanBackends) throw new Error("Backend scanning is unavailable");
  return { items: await ports().scanBackends!() };
}
export async function ResolveAgentBackendCLIPath(input: agent_backend_svc.ResolveCLIPathRequest) {
  return ports().resolveBackendCLIPath?.(input.type, input.deviceId) ?? { path: "", found: false };
}
export async function CancelTestAgentBackend(input: agent_backend_svc.CancelTestBackendRequest) {
  await ports().cancelBackendTest?.(input.requestId);
}
export async function CreateOpenClawAgentBackend(input: agent_backend_svc.CreateBackendRequest, token: string) {
  if (ports().createOpenClawBackend) return { item: await ports().createOpenClawBackend!(input, token) };
  return CreateAgentBackend(input);
}
export async function UpdateOpenClawAgentBackend(input: agent_backend_svc.UpdateBackendRequest, token: string, clearToken = false) {
  if (ports().updateOpenClawBackend) return { item: await ports().updateOpenClawBackend!(input.id, input, token, clearToken) };
  return UpdateAgentBackend(input);
}
export async function TestOpenClawAgentBackend(input: agent_backend_svc.TestBackendRequest, token: string) {
  if (ports().testOpenClawBackend) return ports().testOpenClawBackend!(input, token);
...
export async function RemoteDeviceFingerprint() { return ports().localDeviceFingerprint?.() ?? ""; }
export async function RemoteDeviceList() { return ports().listRuntimeDevices?.() ?? []; }
export async function RemoteDeviceListProviders(deviceID: number) { return ports().listRuntimeDeviceProviders?.(deviceID) ?? []; }
export async function RemoteDeviceSyncProvider(deviceID: number, providerKey: string) { await ports().syncRuntimeDeviceProvider?.(deviceID, providerKey); }
export async function ServerListDevices() { return ports().listAccountDevices?.() ?? []; }
export function EventsOn(_event: string, listener: (payload: unknown) => void) { return ports().onRuntimeDeviceState?.(listener) ?? (() => {}); }
