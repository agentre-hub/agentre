...
export interface ProjectCreateDialogProps {
  ...
  /** 宿主自己的图标选择器（见 `ProjectSettingsDialogProps.iconField`）。 */
  iconField?: (props: {
    value: string;
  ...
  return (
    ...
    >
      ...
      <DialogShellBody className="space-y-4">
        ...
        {iconField ? (
          <div>{iconField({ value: icon, color, onPick: setIcon })}</div>
        ) : null}
          ...
          <div className="mt-1 grid grid-cols-8 gap-1.5">
            {agentColorOrder.map((c) => (
              <button
                ...
                type="button"
                aria-label={c}
                data-testid={`project-create-color-${c}`}
                onClick={() => setColor(c)}
        ...
        {parentOptions.length > 0 ? (
          <label className="block text-xs font-medium text-foreground">
            ...
            >
              <option value="">{t("projectSettings.field.noParent")}</option>
              {parentOptions.map((p) => (
