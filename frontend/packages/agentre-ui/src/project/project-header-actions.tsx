 ...
 * 桌面端的右键菜单就少了当时那两条直落设置的条目而 ⋮ 上有。条目只在 `useMenuItems()`
 * 里定义一次，两种容器各渲染一遍。
 *
...
export interface ProjectHeaderActionsProps {
  ...
  /**
   * 「导入本地会话…」（规格 2026-08-26，决策 13）。**可选即能力开关**：没给这一条
   * 整条不出现 —— 与其余三条轴共用同一份文案与图标（`session-import/import-menu`），
   * 项目组头只是把它插进自己这份全集里。
   */
  onImportLocalSession?(projectId: string): void;
 ...
 * 条目全集与顺序，**只在这里定义一次**。
 *
 * 分组是结构，分隔线是拼接出来的：中间那一组（导入 / 终端 / 合并）三条**全是可选的**，
 * 若把「前面有分隔线」挂在其中某一条上，宿主一条都不声明时菜单就会以贴边的分隔线
 * 收尾、或在删除前排出两条挨着的分隔线。空组在拼之前就被丢掉，分隔线因此只可能
 * 出现在两组之间。
 */
function useMenuItems(p: ProjectHeaderActionsProps): MenuItemSpec[] {
  const { t } = useUiTranslation();
  const importLabel = useImportLocalSessionLabel();

  // 中间那一组：三条各自可选，可能一条都没有。
  const optional: MenuItemSpec[] = [];
  // 与终端一样是「这个项目在磁盘上的事」，所以排在这一组的头一条。
  if (p.onImportLocalSession) {
    optional.push({
      id: IMPORT_MENU_ITEM_ID,
      label: importLabel,
      icon: <ImportLocalSessionIcon />,
      run: () => p.onImportLocalSession?.(p.projectId),
    });
  }
  ...
  const groups: MenuItemSpec[][] = [
    [
      {
        id: "settings",
        // 弹窗现在一屏放得下，成员与机器路径都在里面 —— 菜单不再为它们各留一条
        // 深链，直落某一节的入口只剩「未配置」角标那一个。
