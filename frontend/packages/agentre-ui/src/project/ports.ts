 ...
 * port **交出判别式结果，而不是抛错**。
 *
 * 错误分类是 wire 相关的：`agentre-server` 读的是稳定 RPC 错误码，
...
export interface ProjectMachineView {
   ...
   * 所以默认只列 `isSelf || path || hasMember` 这三类，其余收进「＋ 在别的机器上配
   * 路径」。此前那一节把账号里的每一台 agentred 都摊开：配了八台，一个刚建的项目
   * 就画九行、其中八行是空的。
   *
   * **可选即能力**：宿主答不出「某个成员的 Agent 绑在哪台机器上」时不给它，那一节
   * 退成「本机 + 已配路径的」，功能不减，只是少一档自动带出来的行。清单本身仍然是
   * 全的（`listMachines` 照旧返回账号里的每一台），要不要直接列出来才是这一格决定的
   * —— 包拿得到全集，才画得出那颗「＋」里能挑什么。
   */
  hasMember?: boolean;
}
