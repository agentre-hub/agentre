...
export interface PermissionModePillProps {
  ...
  disabled?: boolean;
  /**
   * 禁用的**原因**，覆盖默认的「生成中不可切换」。
   *
   * 禁用有不止一个来路：轮次进行中不能切（默认那句），以及「这台机器此刻列不出
   * 档位」（能力问不到）。后者若沿用默认文案，界面就会对着一台答不上来的机器说
   * 「生成中不可切换」——一句与事实无关的话。给不出具体原因时留空即可。
   */
  disabledReason?: string;
  /**
   * runtime key(claudecode/codex/builtin/remote)。仅供
   * isPermissionModeDisabled 判定 claudecode bypass-lockout 规则使用。
   * 留空时所有 mode 均可选(不触发 lockout)。
   */
  runtimeKey?: string | null;
  /**
   * 当前会话 spawn 时下发的 permission mode。bypass-lockout 判定输入。
   * 空串/undefined/null = 这条会话还没 spawn 过 CLI(那一刻才落下这个快照),
   * 视作非 bypass 启动。
   */
      ...
      >
        ...
        <ul className="flex flex-col gap-0.5 p-1.5" role="listbox">
          {modes.map((m) => {
            ...
            const isDisabled = isPermissionModeDisabled(m, runtimeKey, ctx);
            // 原因文案在**渲染期**取:元数据表里只有 key,语言切换后跟着变。
            const disabledReason = isDisabled
              ? t(PERMISSION_MODE_DISABLED_REASON_KEY)
              : undefined;
            return (
              <li key={m}>
                <button
