...
describe("computeLiveTurnStats", () => {
  ...
  // 一轮没跑完时,已到的 usage 帧只覆盖到上一次工具调用为止,后面还会再加。
  // 所以「此刻恰好没有未统计的可见文字」不代表数字是终值 —— 只要这一轮还活着,
  // 计数与 tok/s 就都是近似值,`~` 不该在工具空档里闪掉。
  it("Given a tool gap with no visible text yet unfinished turn, When stats are computed, Then completion stays approximate", () => {
    const stats = computeLiveTurnStats({
      model: "claude-sonnet-4",
      now: 20_000,
      startedAt: 1000,
