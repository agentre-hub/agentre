...
describe("useTranscriptScroll", () => {
  ...
  it("Given content grew past the at-bottom tolerance, When a streaming increment lands, Then the follow intent survives and it pins to the freshly measured bottom", () => {
    ...
    // 跟随意图还在 + 存档说刚才贴着底 + 底部被新内容推远 → 这次滚动事件就把它钉回
    // 真实底部(意图驱动;位置说不贴底不作数)。
    expect(loadTranscriptScrollState("tab-follow")?.atBottom).toBe(true);
    expect(el.scrollTop).toBe(8_520);

    act(() => {
      height = 13_000; // 又一段流式增量
      view.rerender(<Harness liveRevision={1} tabKey="tab-follow" />);
    });

    expect(el.scrollTop).toBe(12_520);
  });
  ...
  // 打开会话的那一跳:结构性 follow 按**估值**总高钉底,随后虚拟器逐行复测把总高
  // 顶上去,而虚拟器自己的 anchorTo:"end" 会在复测里把 scrollTop 主动往前推几十像素。
  // 真机实测(sess-3710,179 条消息):钉到 17191 → 总高涨到 18426 → 唯一那次滚动事件
  // 报 scrollTop=17252(= 存档 +61)。位置式判据「scrollTop 还在存档 ±32 内」因此落空,
  // 内容增长被误读成用户上滚 —— 转录区停在离底 700px 的地方,药丸浮出。
  it("Given the transcript pinned to the estimated bottom, When measurement grows the list and the virtualizer has nudged scrollTop past the saved pixel, Then the scroll event re-pins to the real bottom instead of raising the pill", () => {
    let scrollHeight = 4_000;
    const view = render(<Harness messages={[...MESSAGES]} tabKey="tab-grow" />);
    const el = scroller(view.container, () => scrollHeight);
    // 首屏消息到位 → 结构性 follow 钉到当时的底部(估值总高)。
    act(() => {
      view.rerender(<Harness messages={[...MESSAGES]} tabKey="tab-grow" />);
    });
    expect(el.scrollTop).toBe(3_520);

    // 逐行复测:总高长高 1000px,同一批测量里虚拟器把 scrollTop 往前推 61px。
    scrollHeight = 5_000;
    act(() => {
      el.scrollTop = 3_581;
      fireEvent.scroll(el);
    ...
    expect(el.scrollTop).toBe(4_520);
    expect(queryPill()).toBeNull();
  });

  // 同一段窗口里用户真的上滚了:跟随意图解除,内容再长高也不许把他拽回底部。
  it("Given the content grew but the user scrolled up in the same window, Then it keeps the reading position and raises the pill", () => {
    let scrollHeight = 4_000;
    const view = render(
      <Harness messages={[...MESSAGES]} tabKey="tab-grow-up" />,
    );
    const el = scroller(view.container, () => scrollHeight);
    act(() => {
      view.rerender(<Harness messages={[...MESSAGES]} tabKey="tab-grow-up" />);
    });
    expect(el.scrollTop).toBe(3_520);
