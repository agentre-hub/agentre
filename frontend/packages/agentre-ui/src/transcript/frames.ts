 ...
 *                 steer_consumed（轮次在跑时插进来的那句话，一条 steer 一条用户
 ...
 * 往流里推一条用户消息。
 *
 * 它自成一条，且**不吸收**后续块 —— 后面的思考/工具属于助手那一轮。两个来源字段
 * 都要：`sourceDevice` 是发起端指纹，buildSourceByMessageId 拿它跟本机指纹比对 ——
 * **本机发出的那条不该标来源**（server 宿主此前自己建表，不做这个比对，于是你自己
 * 发的消息也挂着「From Chrome on macOS」）。`sourceDeviceName` 只是它的显示名，
 * 没有时回退到指纹本身；报不出来的一律留空，不编一个。
 *
 * 两个调用点（user_message 帧、steer_consumed 里的每条排队消息）说的是同一件事：
 * 用户说了一句话，这一轮的助手段落到此为止。
 */
function pushUserMessage(
  st: State,
  sessionId: number,
  text: string,
  source: { device?: string; deviceName?: string },
): void {
  const msg = emptyMessage(st.nextId++, "user", sessionId, st.frameAt);
  msg.blocks.push({ type: "text", text });
  if (source.device) msg.sourceDevice = source.device;
  if (source.deviceName) msg.sourceDeviceName = source.deviceName;
  st.messages.push(msg);
  st.open = null;
  st.turn = null;
}
...
function applyFrame(
  ...
  switch (kind) {
    ...
    case EventUserMessage: {
      pushUserMessage(st, sessionId, str(ev, "text") ?? "", {
        device: str(ev, "sourceDevice"),
        deviceName: str(ev, "sourceDeviceName"),
      });
      return;
    }

    case EventSteerConsumed: {
      // 轮次在跑时插进来的那句话。它**没有** user_message 帧：`runtime.steer` 只把
      // 文本排进 inbox，被后端消费时对端才发这一条，文本与提交方都在这里面
      // （wire 的 ConsumedSteer）。不认它，拿帧渲染的那两个面（浏览器控制台、
      // 桌面端 Peer Tab）就永远画不出这句话 —— 用户看得见回答，看不见自己问了什么。
      //
      // 落点逐条照搬桌面端 chat_svc 的 persistConsumedSteers：一条 steer 一条用户
      // 消息（不合并 —— 合并是「轮末残留另起一轮」那条路的事）、各带各的来源、空文本
      // 跳过（nonEmptyConsumedSteers）。消费点同时收口当前这条助手消息：后面的正文
      // 属于新的一段，接着往旧段里写会把插话前后糊成一条。
      for (const steer of list(ev, "steers")) {
        const text = str(steer, "text") ?? "";
        if (!text) continue;
        pushUserMessage(st, sessionId, text, {
          device: str(steer, "sourcePeer"),
          deviceName: str(steer, "sourceName"),
        });
      }
      return;
    }
