...
export function useTranscriptScroll(
  ...
  const saveBottomScrollPosition = React.useCallback(
    (metrics: ScrollMetrics) => {
      ...
      // 记下「是我们把它放在这儿的」:nextAutoFollow 靠 prevScrollTop 区分用户上滚
      // 与程序化贴底,少了这一笔,程序化贴底之后用户的第一次上滚会被拿去和过期的
      // 旧值比,判成「往下滚」而不解除跟随意图。
  ...
  const handleTranscriptScroll = React.useCallback(() => {
    ...
    // 「贴底之后内容又长高了」——重新钉回真实底部。
    //
    // 判据是**跟随意图**(autoFollow)加上「上一次存档说我们在底部」,而不是
    // 「scrollTop 还落在存档 ±32 内」:虚拟器的 anchorTo:"end" 会在逐行复测里主动
    // 把 scrollTop 往前推(真机 sess-3710 实测 +61px),位置式判据一旦被推出容差,
    // 就把内容增长误读成用户上滚 —— 打开长会话时转录区停在离底几百像素处、药丸
    // 浮出,正是这条路。意图对增长免疫、只被用户上滚解除(见 nextAutoFollow),
    // 所以上面那次 nextAutoFollow 必须先算:同一窗口里用户真上滚时它已翻 false,
    // 这里就不会把人拽回底部。
    if (
      !atBottom &&
