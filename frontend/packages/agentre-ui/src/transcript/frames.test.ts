...
describe("reduceFrames:消息与正文", () => {
  ...
  it("给定 steer_consumed 帧带排队消息，当归约，则每条各成一条 user 消息，且后续正文另起一条助手消息", () => {
    // 轮次在跑时发出去的那句话走的是 runtime.steer，它**没有** user_message 帧：
    // 被后端消费时对端只发这一条 steer_consumed（载荷里带 text 与提交方）。不认它，
    // 拿帧渲染的那两个面（浏览器控制台、桌面端 Peer Tab）就永远画不出这句话 ——
    // 用户看得到回答，看不到自己问了什么。桌面端自己的会话不受影响：chat_svc 的
    // persistConsumedSteers 早就把它落成 chat_messages 的行，这里是同一件事的帧版。
    //
    // 落点逐条照搬那一份：一条 steer 一条用户消息（不合并）、各带各的来源、
    // 消费点收口当前这条助手消息，后面的正文属于新的一段。
    const msgs = reduceFrames(
      [
        f({ kind: "text_delta", text: "前半段" }),
        f({
          kind: "steer_consumed",
          steers: [
            {
              queuedId: "q1",
              text: "顺便看下这个",
              sourcePeer: "fp-browser-1",
              sourceName: "Chrome on macOS",
            },
            { queuedId: "q2", text: "还有这个" },
          ],
        }),
        f({ kind: "text_delta", text: "后半段" }),
      ],
      SID,
    );
    ...
    expect(msgs[0].blocks).toEqual([{ type: "text", text: "前半段" }]);
    expect(msgs[1].blocks).toEqual([{ type: "text", text: "顺便看下这个" }]);
    expect(msgs[1].sourceDevice).toBe("fp-browser-1");
    expect(msgs[1].sourceDeviceName).toBe("Chrome on macOS");
    expect(msgs[2].blocks).toEqual([{ type: "text", text: "还有这个" }]);
    // 来源报不出来的（本机发的）不编一个：空 sourceDevice 就是「不标来源」。
    expect(msgs[2].sourceDevice).toBeUndefined();
    expect(msgs[3].blocks).toEqual([{ type: "text", text: "后半段" }]);
  });

  // 没有可显示文本的 steer_consumed 仍旧什么都不做，**也不切段**：它此时说的只是
  // 「inbox 空了」。空文本那一条与 chat_svc 的 nonEmptyConsumedSteers 同一条规矩。
  it.each([
    ["没有排队消息", {}],
    ["排队消息是空文本", { steers: [{ queuedId: "q1", text: "" }] }],
  ])("给定 %s 的 steer_consumed 帧，当归约，则不产生任何块", (_name, extra) => {
    const msgs = reduceFrames(
      [
        f({ kind: "text_delta", text: "正文" }),
        f({ kind: "steer_consumed", ...extra }),
...
describe("reduceFrames:消息的时刻", () => {
  /**
   * 每条消息的 `createtime` 取**开启它那一帧**报的时刻。
   *
   * 拿 wire 帧的那两个面没有第二个来源可用：桌面端自己的会话读的是
   * `chat_messages.createtime`，而它们手里只有帧。此前这里一律留 0，于是
   * `formatHHmm(0)` 返回空串，浏览器控制台的转录上**每条消息都没有时间**，
   * 而同一条对话在桌面端是有的。
   */
  it("给定帧带 createtime，当归约，则消息的时刻取开启它那一帧的", () => {
    ...
    expect(msgs[0].createtime).toBe(1700000000111);
    expect(msgs[1].createtime).toBe(1700000005222);
  });
