...
export interface IndexRow {
  ...
  fingerprint: string;
  /** 空串 = 发起端没报过 Agent 标识的会话。 */
  agentSyncId: string;
  /** 空串 = 判不出这条会话归哪个项目（cwd 配不上任何项目路径）。 */
  projectSyncId: string;
  /** 最后活动时间；还没记过活动时间的会话是 0，时间轴上排在最后。 */
  updatedAt: number;
  /**
   * 行标题。没有标题时的退化标题（「工作目录 · 后端 · 状态」）由宿主先算好再进来：
...
export interface IndexGroup {
  ...
  color?: string;
  /**
   * 项目轴上的图标 key（`ProjectNode.icon` 原样带出）。组头画的就是那枚项目字形，
   * 只丢下 key 的话宿主到了组头这一层就再也解不出图标，只剩首字。
   */
  icon?: string;
...
/** 按最后活动时间倒序；没有时间的会话（0）沉底而不是冒到最前面。 */
