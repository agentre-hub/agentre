...
describe("AgentGroupHeader", () => {
  ...
  it("没有 Agent 名的那一组不编一个字母出来，可及名也空着", () => {
    render(
      <AgentGroupHeader
        agent={{ name: "" }}
        label="Unnamed"
  ...
  it("宿主给了去处才摆 ＋：点它开一条对话，而不是把这一组折起来", async () => {
    const user = userEvent.setup();
    ...
    await user.click(
      screen.getByRole("button", { name: "New conversation with Ada" }),
    );
    expect(onNewSession).toHaveBeenCalledTimes(1);
    // ＋ 摆在折叠按钮**外面**：点它不该顺手把这一组收起来。
    expect(onToggle).not.toHaveBeenCalled();
  });

  it("不给去处就不摆 ＋——不摆一个点了没反应的按钮", () => {
    render(
      <AgentGroupHeader
        agent={{ name: "Ada" }}
        expanded
