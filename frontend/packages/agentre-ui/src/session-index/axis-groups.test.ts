...
describe("会话索引的轴投影", () => {
  it("项目轴：项目递归成树，父组在子组之前且带深度；判不出项目的落「未归项目」并排在最后", () => {
    ...
    expect(groups.map((g) => [g.key, g.depth])).toEqual([
      ["p-backend", 0],
      ["p-server", 1],
      // 一条会话都没有，但它在调用方给的名单里：照摆，位置按 sortOrder 不变。
      ["p-lonely", 0],
  ...
  it("项目轴：组头把项目的颜色与图标一起带出来 —— 组头画的就是那枚项目字形", () => {
    // 此前只带 color，icon 在投影里丢掉了：宿主拿着组去画组头时手里没有 key，
    // 于是「设置里选了图标、组头上还是首字」。
    const groups = buildAxisGroups("project", {
      rows: [],
      projects: [
  ...
  it("项目轴：名单里的项目一个不落，空的父带着空的子一起摆（组头本身就是答案）", () => {
    // 刚建出来、还一条对话都没有的项目：它在索引上必须看得见，否则挂在组头上的
    // 「机器与路径…」就没有去处，而没配路径的项目恰恰开不出对话——看不见 = 配不了。
    const groups = buildAxisGroups("project", {
      rows: [],
      projects: [
        { syncId: "p-new", name: "刚建的", sortOrder: 0 },
        {
          syncId: "p-new-child",
          name: "刚建的子项目",
          parentSyncId: "p-new",
  ...
  it("三个兜底组各自独立，不合并成一个「其他」", () => {
    ...
    // 「未命名」是 Agent 轴的兜底，两者不在同一个轴上出现。名单里那三个项目
    // 照摆（都空着），「未归项目」仍排在它们之后。
    expect(groups.map((g) => g.key)).toEqual([
      "p-backend",
      "p-server",
