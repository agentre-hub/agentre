...
describe("TerminalPanel", () => {
  ...
  /*
    非安全上下文（宿主部署在 http://<局域网 IP>:port 上）没有 Clipboard API,
    `navigator.clipboard.writeText(...)` 会**同步抛** TypeError —— 挂在 promise
    上的 .catch 接不到,异常直接从这个 key handler 冒出去,选中的内容也没进剪贴板。
    复制要走共享的复制层,它自己有 execCommand 兜底。
  */
  it("Given no Clipboard API, When Cmd/Ctrl+C copies a selection, Then the shared fallback copies it instead of throwing", async () => {
    removeClipboard();
    const selectedAtCopy = installCopyCommandModel();
    getSelectionMock.mockReturnValue("picked-from-the-terminal");
    try {
      render(
        <TerminalPanel
          terminalID="t1"
      ...
      // 有选区就吞掉这次按键(不发 SIGINT)——复制走没走成不改变这个判断。
      expect(result).toBe(false);
      await vi.waitFor(() =>
        expect(selectedAtCopy).toEqual(["picked-from-the-terminal"]),
      );
    } finally {
      restoreClipboardEnv();
    }
