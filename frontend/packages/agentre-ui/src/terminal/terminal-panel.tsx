...
export function TerminalPanel({
  ...
  useLayoutEffect(() => {
    ...
    term.attachCustomKeyEventHandler((ev) => {
      ...
      // 走共享的复制层（它在非安全上下文下用 execCommand 兜底）：直接摸
      // `navigator.clipboard` 时对象不存在会**同步抛**，异常会从这个 key handler
      // 冒出去。这里保持沉默——终端里复制不该弹 toast。
      void copyTextToClipboard(selection).catch(() => {
