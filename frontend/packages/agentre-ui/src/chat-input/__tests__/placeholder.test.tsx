...
const EMPTY_SOURCES: MentionSources = {
  agents: [],
  projects: [],
  devices: [],
};
...
// 只有设备可 @ 的宿主(例如这台机器没建过 agent)照样接上了 @ 这项能力,
// 占位文案要认它 —— 否则用户看不到提示,却按得出菜单。
const SOURCES_WITH_DEVICE_ONLY: MentionSources = {
  agents: [],
  projects: [],
  devices: [{ kind: "device", refId: 0, label: "工作站", fp: "sha256:a" }],
};
...
describe("AIChatInput 的占位文案", () => {
  ...
  it("mentionSources 非空才提 @", () => {
    ...
    const { container: withDevice } = render(
      <AIChatInput
        onSubmit={() => {}}
        mentionSources={SOURCES_WITH_DEVICE_ONLY}
      />,
    );
    expect(placeholderOf(withDevice)).toBe("Type a message · @ to mention");
  });
