...
describe("MarkdownText + mention decorator", () => {
  ...
  // 转录区的 chip 只剩 title 能说明「这个 @ 是什么」—— 设备走的是设备那一条,
  // 不能落回 agent 的 title(那会把一台机器说成一个 agent)。
  it("Given a sent device mention, When the transcript renders it, Then the chip names it as a device", () => {
    const { text, refs } = prepareMentionText(
      '<device fp="sha256:ab12">工作站</device>',
    );
    expect(refs).toEqual([
      { kind: "device", refId: 0, label: "工作站", fp: "sha256:ab12" },
    ]);
    render(<MarkdownText text={text} decorator={makeMentionDecorator(refs)} />);
    expect(screen.getByText("@工作站")).toHaveAttribute(
      "title",
      "Device: 工作站",
    );
  });
