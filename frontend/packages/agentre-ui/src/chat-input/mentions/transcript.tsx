...
// 三个键写死成字面量而不是按 kind 拼接 —— 键要能被静态扫出来。
function chipTitle(
  ref: MentionRef,
  t: (key: string, opts: { name: string }) => string,
): string {
  if (ref.kind === "agent")
    return t("mentions.chip.agentTitle", { name: ref.label });
  if (ref.kind === "project")
    return t("mentions.chip.projectTitle", { name: ref.label });
  return t("mentions.chip.deviceTitle", { name: ref.label });
}
...
export function MentionChip({
  ...
  const title = chipTitle(refData, t);
