...
describe("AIChatInput @ mention integration", () => {
  ...
  it("picking a device inserts a chip that serializes to its fingerprint on submit", async () => {
    const onSubmit = vi.fn();
    render(<Harness onSubmit={onSubmit} />);

    await pickMention("工作站");

    act(() => screen.getByTestId("submit").click());
    expect(onSubmit).toHaveBeenCalledWith(
      expect.stringContaining('<device fp="sha256:ab12">工作站</device>'),
    );
  });
