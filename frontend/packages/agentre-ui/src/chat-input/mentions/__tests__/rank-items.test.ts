...
describe("rankMentionItems", () => {
  ...
  it("Given devices, when ranked, then they follow Projects and keep the order the host handed over", () => {
    const sources: MentionSources = {
      agents: [{ kind: "agent", refId: 1, label: "Reviewer" }],
      projects: [{ kind: "project", refId: 2, label: "Web" }],
      devices: [
        // 宿主把本机摆在最前;打平时 rankGroup 按 sourceIndex 稳定,所以这个顺序要留住。
        { kind: "device", refId: 0, label: "本机", fp: "sha256:self" },
        { kind: "device", refId: 0, label: "工作站", fp: "sha256:ws" },
      ],
    };

    expect(rankMentionItems(sources, " ").map((item) => item.label)).toEqual([
      "Reviewer",
      "Web",
      "本机",
      "工作站",
  ...
  it("Given a device query, when it matches only a device label, then only that device comes back", () => {
    const sources: MentionSources = {
      agents: [{ kind: "agent", refId: 1, label: "Reviewer" }],
      projects: [],
      devices: [
        { kind: "device", refId: 0, label: "工作站", fp: "sha256:ws" },
        { kind: "device", refId: 0, label: "NAS", fp: "sha256:nas" },
      ],
    };

    expect(rankMentionItems(sources, "nas")).toEqual([sources.devices[1]]);
  });
  ...
  it("Given an empty query or a group with no matches, then source order is stable and empty groups disappear", () => {
    ...
    expect(
      rankMentionItems({ agents: [], projects: [], devices: [] }, "anything"),
    ).toEqual([]);
  });
});
