 ...
 *
 * **key → 图标节点这一步也在这里**（`iconNode`）：它不需要任何宿主知识，留在宿主
 * 的后果是「词表在包里、解 key 在宿主」，谁忘了接谁就永远只画得出首字母。
 * **选择器**仍是宿主各画各的：桌面端的头像还有「上传图片」这一档（浏览器宿主的
 * 读写端点都不带头像正文，画一个上传按钮就是伪造后端）。
 *
...
/**
 * icon key → 字形方块里画好的那枚图标节点；**没有对应图标时不给节点**，
 * 让调用方退回首字母。
 *
 * 与 `iconForKey` 的拼图兜底刻意不同：那一档是给选择器用的（格子里总得画点什么），
 * 而字形里拿拼图顶上去会让用户以为自己设过一枚拼图。
 *
 * 这一步此前只在桌面端有（`primitives.tsx` 的 `agentIconNode`），词表却早已搬进
 * 这个包 —— 于是浏览器宿主拿得到词表、画不出图标。渲染 lucide 组件不需要任何
 * 宿主知识，所以它跟着词表一起归包。
 */
export function iconNode(
  ...
  if (!hasIcon(key)) return undefined;
  return React.createElement(iconForKey(key), {
    className,
    "aria-hidden": true,
  });
}
