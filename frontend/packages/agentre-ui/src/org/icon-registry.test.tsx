...
/**
 * `iconNode` 是「key → 画好的节点」这一步的**唯一**实现。此前它只在桌面端有
 * （`primitives.tsx` 的 `agentIconNode`），词表却早已搬进这个包 —— 于是浏览器宿主
 * 拿得到词表、画不出图标，项目字形与 Agent 头像一律退回首字。
 */
describe("iconNode", () => {
  it("Given 词表里的 key, When 取节点, Then 画出对应的那枚 lucide 图标", () => {
    const { container } = render(iconNode("rocket") as React.ReactElement);

    const svg = container.querySelector("svg");
    expect(svg?.getAttribute("class")).toContain("lucide-rocket");
    // 字形方块里占六成宽高——两端此前各自写死的也是这一档。
    expect(svg?.getAttribute("class")).toContain("size-[60%]");
    // 旁边一定有名字（项目名 / Agent 名），图标只是装饰。
    expect(svg?.getAttribute("aria-hidden")).toBe("true");
  });

  it("Given 空 key 或词表不认得的 key, When 取节点, Then 没有节点（调用方退回首字）", () => {
    // 与 `iconForKey` 的拼图兜底刻意不同：那是「选择器里得画点什么」，
    // 这里是「字形该退回首字母」——拿拼图顶上去会让用户以为自己设了个拼图。
    expect(iconNode("")).toBeUndefined();
    expect(iconNode(null)).toBeUndefined();
    expect(iconNode("nope")).toBeUndefined();
  });
});
