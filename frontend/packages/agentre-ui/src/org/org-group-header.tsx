...
import { OrgIndentRails, orgHeaderPaddingLeft } from "./org-indent";
...
export function OrgGroupHeader({
  ...
  return (
    <div
      ...
      style={{ paddingLeft: orgHeaderPaddingLeft(group.depth) }}
      className={cn(
        // relative 是给缩进导引线的定位基准（见 org-indent.tsx）。
        "relative flex items-center gap-1.5 rounded-md py-[5px] pr-1.5 text-xs font-semibold transition-colors",
        // 顶层部门之间留一口气：分的是「一个部门一块」，所以只给 depth 0；子部门
        // 头留白会把它从父部门那块里推出去，读成又一个顶层部门。
        group.depth === 0 && "mt-[7px]",
    ...
    >
      <OrgIndentRails depth={group.depth} />
      {dragHandle ? (
        <button
          type="button"
          ref={handleRef}
      ...
      >
        {/* 组头与行同字号，字距是把「部门名」和「Agent 名」拉开的最后一档。 */}
        <span className="truncate tracking-[0.045em]">{department.name}</span>
