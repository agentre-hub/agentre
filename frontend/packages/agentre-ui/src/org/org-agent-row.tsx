...
import { OrgIndentRails, orgRowPaddingLeft } from "./org-indent";
...
export function OrgAgentRow({
  ...
  return (
    <div
      ...
      data-indent={indent}
      style={{ paddingLeft: orgRowPaddingLeft(indent) }}
      data-drop-kind={droppable ? "agent" : undefined}
      data-drop-state={dropState}
      aria-current={selected ? "true" : undefined}
      className={cn(
        // relative 是给缩进导引线的定位基准 —— 线在内容左边的空槽里，不压到字。
        "relative flex items-center gap-[7px] rounded-md py-[5px] pr-2 text-xs transition-colors",
