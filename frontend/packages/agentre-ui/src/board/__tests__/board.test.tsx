...
describe("卡片标题", () => {
  it("Given a long unbreakable title, When the card renders, Then the title can break so the two clamped lines stay readable", () => {
    // 卡片定宽,标题 line-clamp 两行。没有 overflow-wrap 时,一条没有空格的长标题
    // (测试名 / 路径 / URL)不折行,被 clamp 的 overflow:hidden 从词中间裁掉 ——
    // 既不换行也不出省略号。jsdom 不做布局,这里守类名。
    const title =
      "TestRelayClient_GivenAReservedChannelID_ThenTheClientCannotConnect";
    renderBoard(viewModel({ columns: { todo: column([card(1, { title })]) } }));
