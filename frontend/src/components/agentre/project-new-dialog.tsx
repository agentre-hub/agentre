 ...
 *
 * 表单本身住在 `@agentre-hub/agentre-ui` 里，两端同一份。这里做的只有：把项目树拍平
      ...
      create: async (draft) => {
        try {
          const created = await ProjectCreate({
            ...
            name: draft.name,
            // 没挑图标就送空串：`"folder"` 不在共享词表里（`hasIcon` 认不得），
            // 落库之后侧栏的字形照样退回项目名首字 —— 存一个画不出来的值只是
            // 让「它有图标」这句话在数据里成立而已。`Project.Check` 也只校验
            // 颜色，不要求 icon 非空。
            icon: draft.icon ?? "",
