...
// agentBackendId 是**夹具自己的**便捷参数：OrgAgent 上后端与技能授权的唯一出口是
// execTargets（R15/R15e），单档场景由它派生出那一条。
function agent(
  p: Partial<OrgAgent> & { id: number; agentBackendId?: number },
): OrgAgent {
  return {
    id: p.id,
    name: p.name ?? `a${p.id}`,
    ...
    // 「一档都没有」因此是可信的事实而不是缺省。
    execTargets: p.execTargets ?? [
      { id: 1, agentBackendId: p.agentBackendId ?? 5, skills: [] },
    ],
...
describe("OrgIndex 组头与行", () => {
  ...
  it("行缩在自己的部门头下一级，部门外的行不缩", () => {
    setup();

    // 部门里的行比组头缩一级 —— 否则成员与部门名挤在同一条左缘，读不出归属。
    expect(screen.getByTestId("org-group-1")).toHaveAttribute(
      "data-depth",
      "0",
    );
    expect(screen.getByTestId("org-row-2")).toHaveAttribute("data-indent", "1");
    // 系统 Agent 不属于任何部门，与顶层部门头平齐。
    expect(screen.getByTestId("org-row-1")).toHaveAttribute("data-indent", "0");
  });
  ...
  it("下属 Agent 与主管平排，行内带 ↳ 主管", () => {
    ...
    // 平排：下属行不靠缩进表达从属，缩进只跟着所在部门走 —— 主管与下属同为 1。
    expect(bob).toHaveAttribute("data-indent", "1");
    expect(screen.getByTestId("org-row-2")).toHaveAttribute("data-indent", "1");
    expect(screen.getByTestId("org-row-2")).not.toHaveTextContent("↳");
    // 子部门里的行跟着子部门缩一级（层级只剩「部门套部门」这一种）
    expect(screen.getByTestId("org-row-6")).toHaveAttribute("data-indent", "2");
  });
