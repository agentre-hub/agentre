...
describe("接缝", () => {
  ...
  it("没挑颜色 / 图标时 adapter 补上这一端后端要的默认值", async () => {
    ...
    await waitFor(() =>
      // Project.Check 只认调色板里的颜色，空串会被后端拒 —— 所以颜色有默认值。
      // 图标没有：`Check` 不校验它，而补一个词表里没有的 key（此前是 "folder"）
      // 只会落一个画不出来的值。
      expect(appMocks.ProjectCreate).toHaveBeenCalledWith(
        expect.objectContaining({ color: "agent-1", icon: "" }),
      ),
    );
  });
