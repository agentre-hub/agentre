...
export type IndexGroupHandlers = {
  ...
  onOpenNotChattable: (agent: AgentSlim) => void;
  ...
  /**
   * 打开「导入本地会话」对话框，按这一维预填（决策 13）。
   *
   * **可选 = 能力开关**：不给这条 handler，四个组头上的那条菜单条目整条不出现。
   */
  onImportLocalSession?: (prefill: ImportDialogPrefill) => void;
...
export function IndexGroupRow({
  ...
  const overflow =
    !visibleSessionIDs && group.total > group.sessionIDs.length
      ? group.total
      : undefined;
  // 「自己的会话」组头写的是**这个项目有几条会话**，不是首屏拉回来几条：后者只是
  // GROUP_PAGE_SIZE 那个窗口，会让 44 条的项目顶着「会话 5」，与紧挨着它的
  // 「查看全部 44」自相矛盾。筛选生效时回到「筛出来几条」—— 那时总数不是屏幕上
  // 这一段的口径（与上面的 overflow 同一条理由）。
  const ownSessionsCount = visibleSessionIDs
    ? sessions.length
    ...
    return (
      <AgentGroup
        ...
        headerActions={
          <ImportLocalSessionMenu
            label={agent.name}
            testId={`import-menu-agent-${agent.id}`}
            onImport={
              handlers.onImportLocalSession
                ? // Agent 组头预选导入目标：backend / provider / model 跟着它定，
                  // 底部那一步只剩 cwd 要确认（板 E3）。
                  () =>
                    handlers.onImportLocalSession?.({
      ...
      return (
        <MachineGroupHeader
          ...
          actions={
            <ImportLocalSessionMenu
              label={machine.name}
              testId={`import-menu-machine-${machine.deviceId}`}
              onImport={
                handlers.onImportLocalSession
                  ? // 机器组头进入即锁定那台机器：导的是**那台**机器磁盘上的会话，
                    // 导完续跑也在那台机器上。
                    () =>
                      handlers.onImportLocalSession?.({
    ...
    if (group.kind === "project" && project) {
      return (
        <ProjectGroupHeader
          ...
          onImportLocalSession={
            handlers.onImportLocalSession
              ? // 项目组头预填该项目的本机路径：路径已知 → 候选默认只列这个 cwd 的，
                // 零筛选；导入后这条会话也归到这个项目。
                () =>
                  handlers.onImportLocalSession?.({
                    scopeLabel: project.name,
                    cwdPrefix: project.path,
