...
/** agent 名下已知的会话 id：前 5 条 + attention，去重后按最近活动排。 */
function agentSessionIDs(
  agent: { sessions?: { id: number }[]; attentionSessions?: { id: number }[] },
  metas: ReadonlyMap<number, { lastMessageAt?: number }>,
): number[] {
...
export function useIndexGroups(
  ...
  return React.useMemo(() => {
    ...
    // 「随手对话」常驻，空态也在（决策 6）：否则一条自由会话都没有的用户，在项目轴
    // 下看不到任何「不属于项目」的入口。它排在全部项目之后。
