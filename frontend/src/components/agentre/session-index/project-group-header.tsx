...
export type ProjectGroupHeaderProps = {
  ...
  /** 「导入本地会话…」。可选即能力开关：不给就整条不出现（规格 2026-08-26）。 */
  onImportLocalSession?: () => void;
};
...
export function ProjectGroupHeader({
  ...
  */
    ...
    async (projectID: string): Promise<ProjectHeaderMember[]> => {
      ...
      return rows.map(({ m, inherited }) => {
        ...
        return {
          ...
          avatarIcon: iconNode(m.avatarIcon || agent?.avatarIcon || ""),
  ...
  const actions: ProjectHeaderActionsProps = {
    ...
    onImportLocalSession: onImportLocalSession
      ? () => onImportLocalSession()
      : undefined,
  };
