...
describe("SessionIndexPage search and filter chips", () => {
  ...
  // 弹层的组头此前只认 agent / free / 项目三档，机器组落进项目那一支：deviceID 被
  // 当成项目 id 去查名字 —— 本机（0）查不到就成了空标题，恰好撞上某个项目 id 的
  // 机器则顶着那个项目的名字。
  it("Given the machine axis, When 「查看全部」 opens, Then the popover header names that machine", async () => {
    const many: SeedSession[] = Array.from({ length: 8 }, (_, i) => ({
      id: 400 + i,
      title: `on this box ${i}`,
      agentId: 7,
    ...
    const user = setupUser();
    renderIndex();
    await user.click(await screen.findByText("View all 8 sessions"));

    const popover = await screen.findByRole("dialog");
    expect(within(popover).getByText("This machine")).toBeInTheDocument();
  });
...
describe("SessionIndexPage data path", () => {
  ...
  it("Given the whole front end, When sources are scanned, Then attention is computed in exactly one place (problem 3)", () => {
    ...
    // （规格问题 3）。收敛之后它必须只有一处**实现** —— 扫源码文本，因为第二处
    // import 在类型上完全合法，编译期与 ESLint 都不会红。
    //
    // 那一处现在在共享包里：agentre-server 问的是同一个问题，而它此前各画了一份
    // 缩水的（`toAgentStatus` 只是 `reasonToDisplayStatus` 的一半）。宿主这边只剩
    // 转出去的那一行，收敛没有变松 —— 断言因此分成两半：实现恰好一个文件，其余
    // 提到这个名字的地方必须是 re-export，不能是第二份 `function`。
    //
    ...
    for (const root of ["src", "packages/agentre-ui/src"]) {
      for (const file of walkSources(resolve(process.cwd(), root))) {
        ...
        mentions.push(where);
        if (/function\s+computeAttention\b/.test(code))
          declarations.push(where);
      }
    }

    expect(declarations).toEqual([
      "packages/agentre-ui/src/session-index/attention.ts",
    ]);
    expect([...mentions].sort()).toEqual([
      "packages/agentre-ui/src/index.ts",
      "packages/agentre-ui/src/session-index/attention.ts",
      "src/stores/attention-store.ts",
    ]);
  });
});
