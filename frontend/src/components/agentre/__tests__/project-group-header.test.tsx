...
describe("ProjectGroupHeader ⋮ menu", () => {
   ...
   * 菜单不再为它们各留一条深链。直落某一节的入口只剩「未配置」角标那一个。
   */
  it("Given the ⋮ menu, Then the two section deep links are gone — settings opens at the top", async () => {
    const user = setupUser();
    ...
    await screen.findByRole("menuitem", { name: "Project settings…" });
    expect(
      screen.queryByRole("menuitem", { name: "Machines and paths…" }),
    ).toBeNull();
    ...
    expect(props.onOpenSettings).toHaveBeenCalledWith(1, undefined);
  });
});
...
describe("ProjectGroupHeader context menu", () => {
  ...
  it("Given the header is right-clicked, Then the menu is identical to ⋮ — one definition, two containers", async () => {
    ...
    /*
      ...
      那两条深链本身已经并回「项目设置…」（2026-08-27），但这条守卫要守的是
      「两个容器同一份」，与条目多少无关。
    */
    expect(menuItemLabels()).toEqual([
      "Project settings…",
