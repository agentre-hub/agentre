...
describe("桌面端接上了包的宿主样式基线", () => {
  it("globals.css import 了 base.css 与 toast.css", () => {
    ...
    expect(css).toMatch(/@import\s+"@agentre-ai\/agentre-ui\/base\.css"/);
    expect(css).toMatch(/@import\s+"@agentre-ai\/agentre-ui\/toast\.css"/);
  });
  ...
  it("vite 给这两个入口配了别名 —— 少了它桌面端解析不到，且不报错", () => {
    ...
    expect(config).toContain("@agentre-ai/agentre-ui/base.css");
    expect(config).toContain("@agentre-ai/agentre-ui/toast.css");
  });
