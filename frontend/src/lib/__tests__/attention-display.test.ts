...
it("pill text for bg_running is the background label", () => {
  // 断的是文案本身，与上面那一组 pill 同一种写法。此前这里拿
  // `i18n.t("attention.background")` 当期望值：那些 key 已经随判定与投影搬进共享包
  // 的 `agentreUi` namespace，落到宿主 defaultNS 上取不到，i18next 会把 key 原样回显
  // ——于是期望值变成了 "attention.background" 这个字符串本身，两边一起错才叫过。
  expect(reasonToPillText("bg_running")).toBe("Background");
});
