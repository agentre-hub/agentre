...
// 补丁或回填迁移。
//
// 2026-09-10 为 0.1.0 首发又做了一次同样的收口：把三条只补齐别人漏掉的东西的补丁折进
// 各自领域的建表语句，补丁整条退役：
//
//   - 202609070101（project_locations 补 device_id）→ 折进 202609040105
//   - 202609080101（六张表补自增 id 主键）→ 折进 202609040105 / 106 / 108 / 109 /
//     110 与 202609060101
//   - 202609090101（chat_messages 补 turn_trigger）→ 折进 202609040106
//
// 折叠的等价性是拿库比的：同一套迁移跑出的旧库与新库，逐表列/索引/外键/CHECK/自增
// 与种子行完全一致。新增表那两条（202609060101 / 202609080201）不动 —— 它们不是补丁，
// 只是晚一点才出现的一个领域，折进去除了多退役两个号之外什么也换不来。
//
