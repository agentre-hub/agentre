...
// migration202609080201 建 port_forwards 表:桌面端作为被访问设备时(局域网直连、或
// 被另一台桌面端访问),记着自己声明过的端口转发映射(规格 2026-09-06「设备端口
// 转发」的「数据」一节)。
//
// DDL 逐字取自 agentred 一侧(internal/daemon/migrations/202609080201_port_forward.go)
// ——两个宿主共用同一份实体(internal/model/entity/port_forward_entity)与仓储
// (internal/repository/port_forward_repo)代码,表结构错一格就是同一行代码在两台
// 机器上写出两种结果。两个进程各一个库,所以这里同样是**复制 DDL**而不是共享
// 一张表(同形由 internal/daemon/migrations/port_forward_parity_test.go 钉住)。
//
