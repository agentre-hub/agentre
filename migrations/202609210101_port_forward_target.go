...
// 一节)。
//
// DDL 逐字取自 agentred 一侧(internal/daemon/migrations/202609210101_port_forward_target.go)
// ——两个宿主共用同一份实体(internal/model/entity/port_forward_entity)与仓储
// (internal/repository/port_forward_repo)代码,表结构错一格就是同一行代码在两台
// 机器上写出两种结果(同形由 internal/daemon/migrations/port_forward_parity_test.go
// 钉住)。
//
