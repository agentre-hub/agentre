...
- **Repo unit tests: always go through `testutils.Database(t)`, never a real SQLite.** Take the `(ctx, _, mock)` triple; the business code automatically hits the mock via `db.Ctx(ctx)`. Each case asserts SQL and parameters precisely with `mock.ExpectQuery / ExpectExec`, and always ends with `assert.NoError(t, mock.ExpectationsWereMet())`. Note that **`testutils.Database` uses the MySQL dialect**, so use `` `table_name` `` backticks in the regex match; GORM `Create / Save / Update` opens a transaction automatically by default, so the match template writes `ExpectBegin / ExpectExec / ExpectCommit` (add `ExpectRollback` for the error path). **Exceptions** — exactly two places may use `t.TempDir()` to spin up a real SQLite; every other repo / service test goes through a mock:
  ...
  2. `internal/daemon/daemon_test.go` — `agentred` opens and owns its own database, and these cases assert facts that only a real engine has. The daemon session upsert's `ON CONFLICT` target has to land verbatim on a PRIMARY KEY or UNIQUE constraint, or no session can be created at all; sqlmock replays a scripted expectation list in a MySQL dialect — where GORM renders that upsert as a column-less `ON DUPLICATE KEY UPDATE` against no schema — and cannot observe that. Same for the WAL-vs-rollback-journal case (a catch-up read must not stall the streaming writer) and the reported database file size. The daemon's *repository* tests (`internal/daemon/repository/*`) are not exempt and do use sqlmock.
...
| `internal/pkg/agentruntime/runtimes/remote/wire/tsgen_test.go` (`TestTSGenCoversHostContractMethods`) | The generated host contract (`host-contract.gen.ts`, which host answers which method) covers every `wireinbound.Contract()` row, and each mapped `rpcMethods` key really exists in `rpc-methods.ts` **with the descriptor ID the Go enum declares** — a misspelled or misassigned key would silently drop a method from the consumer's list with no other signal. Read by AST, not by import: `wireinbound` depends on the `wire` package, so the generator cannot import it (same wall as `TestTSGenCoversChatBlockTypes`) |
