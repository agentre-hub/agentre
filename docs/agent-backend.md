...
| Remote daemon | ✅ | ✅ | ✅ | ✅ | ❌ | ❌ (refused: a hermes backend has no LAN `agentred` dispatch path) | ✅ | the runtime must not depend on desktop-only services (chat_repo / GUI); OpenClaw is blocked by missing daemon-local secret enrollment/reference |
...
2. **Secret boundary** — the refresh token is stored through `internal/pkg/keychain` under an account derived from the normalized URL, so one serve shares one credential and changing the URL forces a new sign-in. The password is used once and never persisted or logged; deleting the backend deletes the credential.
3. **Protocol client** — `internal/pkg/agentruntime/runtimes/hermes/{client.go,frame.go,auth.go}` owns the WebSocket dial, the loopback token handshake, the gated bearer → one-shot 30-second `ws-ticket` handshake, and failure classification (`ErrGatewayUnreachable` / `ErrLoginRequired` / `ErrLoginExpired` / …). `auth.go` implements the PKCE native flow (authorize → password-login → native token → refresh) and never opens a browser or a local listener.
...
6. **Prober / Test Connection** — `hermes.Probe` reuses the same dial path and stops after `gateway.ready`; it does not run a turn. A gated serve answers `GET /` with a 302 to its login page, so "needs login" is detected by "the page carries no session token" — **not** by a 401 and **not** by any marker element.
...
8. **Remote boundary** — never dispatched to `agentred`: `exec_target_svc` blocks a remote-bound hermes backend, and `chat_svc` returns a hint error (`ChatBackendHintRemoteHermes` / `ChatBackendRemoteHermesUnavailable`) instead of failing silently.
