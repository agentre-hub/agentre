...
| Reverse Q&A | not supported | control_request `can_use_tool` + `AskUserQuestion` tool | app-server `item/tool/requestUserInput` JSON-RPC | not supported | not supported | not supported | `session/request_permission` JSON-RPC (select by option **kind**; no deny-reason feedback channel) | claudecode uses the question text as the key, codex uses the question ID as the key; OpenClaw ask-user is unavailable |
...
4. **Runtime** — one WebSocket per turn; the connection is the per-session ownership unit the fence guards. The session is created-or-resumed and the submitted session id returns as `RunResult.ProviderSessionID` for the next turn. Abort sends `session.interrupt`. Only `CapAbort` is declared.
5. **Translator** — a pure function from gateway frames (`message.start|delta|interim|complete`, `thinking.delta`, `reasoning.delta`, `tool.start|generating|complete`, `todo.updated` → `PlanUpdated`, `session.usage`) to sealed events. `approval.request`, `clarify.request` and every unknown frame produce no events (the raw-frame sink logs them), so a frame we do not understand can never wedge the turn.
6. **Prober / Test Connection** — `hermesgateway.Probe` (re-exported as `hermes.Probe`) reuses the same dial path and stops after `gateway.ready`; it does not run a turn. A gated serve answers `GET /` with a 302 to its login page, so "needs login" is detected by "the page carries no session token" — **not** by a 401 and **not** by any marker element.
7. **Frontend** — settings render a single Server URL plus the auth block (provider list, sign in / sign out, signed-in user) with bilingual i18n. Providers that do not support password sign-in are listed but disabled with readable copy; OAuth is not implemented.
8. **Remote boundary** — `agentred` answers the device credential RPCs for a serve: list providers, log in (the password is used for that request only), log out, login status, and a connection test through `hermesgateway.Probe` with the daemon's stored login, returning the desktop's result codes (`HERMES_LOGIN_REQUIRED` / `HERMES_LOGIN_EXPIRED` / `HERMES_UNREACHABLE` / …). Chat turns are still never dispatched to `agentred`, where the runtime is not registered: `exec_target_svc` blocks a remote-bound hermes backend, and `chat_svc` returns a hint error (`ChatBackendHintRemoteHermes` / `ChatBackendRemoteHermesUnavailable`) instead of failing silently.
