...
The repository currently has six backend types, with clearly distinct roles. **Pick the right slot for your new backend before you start** by mapping it to one of these:
...
- The `Capabilities()` implementation in `internal/pkg/agentruntime/runtimes/{builtin,claudecode,codex,piagent,openclaw,hermes}/runtime.go`
...
| Capability (constant / wire string) | Sub-interface | builtin | claudecode | codex | piagent | openclaw | hermes | Description |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
| `CapAbort` / `"abort"` | `Aborter` | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | the "Stop" button; must be idempotent + must unblock all blocking I/O, otherwise the frontend will be stuck on "generating" forever. OpenClaw uses `chat.abort {sessionKey,runId}` |
...
| Dimension | builtin | claudecode | codex | piagent | openclaw | hermes | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Run shape | in-process (cago app/coding + LLMProvider) | CLI subprocess (stdout JSONL) | CLI subprocess (JSON-RPC over stdio app-server) | CLI subprocess (Pi RPC mode) | Gateway WebSocket RPC 4 | WebSocket JSON-RPC to an already-running `hermes serve` (Agentre never starts it) | determines the Prober / cli_path validation / transport ownership path |
| ProviderType binding | any LLM provider | Anthropic family (incl. gateway proxy) | OpenAI / Codex family | not bound to a provider; reads `~/.pi/agent` | Gateway-owned agent/model/auth | not bound: Hermes owns provider/model/auth; `ProviderTypeMatch` false | the entity `BackendKind.ProviderTypeMatch` implementation; piagent and OpenClaw always return false |
| Session mode | turn-scoped cago `Runner`, destroyed when the turn ends | long-lived subprocess + LRU cache reuse, reusing the session across turns | new process per turn, no local reuse | new Pi client per turn; Pi stores native JSONL in its configured/default root and Agentre resumes by the persisted native Session ID | stable `agentre:<backendID>:<sessionID>` key | one WebSocket per turn; Hermes owns session storage, Agentre resumes by the persisted native session id | reuse means managing idle evict / abort unlocking / cross-turn state; OpenClaw returns the stable key as `ProviderSessionID` |
| Translator | pure function (cago events → sealed) | pure function + `task_aggregator` maintaining the Subagent list across turns | pure function | pure RPC mapper + turn-local subagent tracker in the `Run` drain loop | run/session/seq filter + payload translator | pure function (gateway frame → sealed event) | state aggregation stays at the drain boundary and the translator itself must remain independently table-testable; OpenClaw reconnect aggregation belongs in the runtime, not service/repository |
| Plan source | does not emit | `TodoWrite` tool inline (canonical) + `Task*` incremental aggregation (PlanUpdated snapshot) | `turn/plan/updated` (Steps) + `item/plan/delta` (Text) merged into a single PlanUpdated | does not emit | does not emit | `todo.updated` → PlanUpdated | what downstream chat_svc sees is the same sealed `agentruntime.PlanUpdated`; OpenClaw plan capability is not claimed |
| Reverse Q&A | not supported | control_request `can_use_tool` + `AskUserQuestion` tool | app-server `item/tool/requestUserInput` JSON-RPC | not supported | not supported | not supported | claudecode uses the question text as the key, codex uses the question ID as the key; OpenClaw ask-user is unavailable |
...
| Field | builtin | claudecode | codex | piagent | openclaw | hermes | Field meaning |
| --- | --- | --- | --- | --- | --- | --- | --- |
