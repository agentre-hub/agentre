...
9. **Remote boundary** — dispatchable to `agentred` like any local CLI subprocess: `exec_target_svc` treats it as chattable on both local and remote, `chat_svc` resolves it without touching providers/gateways, and the daemon registers the runtime via its blank import. The MCP-injection caveat above is the only behavioral difference from local. The two launch fields travel every cross-machine path like the other identity fields: the syncwire `AgentBackendPayload.config` object (`acpCommand` / `acpArgs`, camelCase like the rest of `AgentBackendConfig` and the desktop's `config_json` column), the daemon protobuf `AgentBackend` message (`acp_command = 28` / `acp_args = 29`, mapped in `protowire.BackendToProto` / `BackendFromProto` — the daemon builds its entity row solely from `BackendFromProto`, so a missing mapping surfaces as `backend has no acpCommand configured` on the remote box), and the local Create/Update DTOs (`acpCommand` / `acpArgs`). They are account-level identity: there is deliberately **no per-device override** for them (unlike `cli_path`, acp has no "known CLI" whose path could differ per machine).
