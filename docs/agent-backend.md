...
1. **Run mode**: is it in-process (like builtin, directly consuming an LLMProvider through cago app/coding), does it wrap a local CLI subprocess (like claudecode / codex / piagent), or does it drive a **separately started service** the user runs and stops themselves (like openclaw's Gateway / hermes' `serve`)? The three shapes are completely different in ProviderType matching, cli_path validation, env passthrough, process ownership, and Prober implementation — the third one has no local process at all, so "who starts it" must be settled explicitly.
...
The repository currently has seven backend types, with clearly distinct roles. **Pick the right slot for your new backend before you start** by mapping it to one of these:
...
- **piagent** — wraps the local `pi` CLI (Pi coding agent RPC mode); it is not bound to an Agentre `LLMProvider`, but reads Pi's own `~/.pi/agent` config and auth. Supports steer / abort / compact / image input / context-window reporting / fork-session plus structured official/flat-single extension subagent streaming and replay; Pi owns the Session storage location, while Agentre persists the native Session ID returned by RPC plus each turn's native user-entry anchor, and resumes with `--session <id>`. It does not support tool approval, reverse Q&A, or permission mode meta.
- **openclaw** — connects directly to OpenClaw Gateway WebSocket RPC protocol 4. It is not a CLI/ACP/HTTP adapter and is not bound to an Agentre `LLMProvider`. It supports local Gateway discovery, stable per-chat session keys, streamed text/thinking/tool/usage events, abort, reconnect reconciliation, a backend-neutral approval lifecycle spanning exec/plugin/system-agent, reverse Q&A via `question.*`, and keeping a yielded (`yielded:true`) turn open until its follow-up run ends. A backend bound to an `agentred` keeps its Gateway token in that daemon's `state.json` (enrolled through the device credential RPCs, [§2.8](#28-openclaw-gateway-backend-implemented-reference)), but the runtime is not registered on `agentred`, so remote execution is still unavailable.
- **hermes** — connects to an **already-running** `hermes serve` over its TUI-gateway JSON-RPC (newline-delimited JSON over a WebSocket; the gateway serves the same dispatch over stdio and WS). It is not a CLI wrapper and is not bound to an Agentre `LLMProvider` — Hermes owns provider/model/auth. The backend carries exactly one transport field, `hermesUrl`; a loopback serve needs no login, while a gated (non-loopback) serve authenticates through the PKCE native flow and keeps only the refresh token in the OS keychain (`hermesAuthProvider` / `hermesUserId` are display-only). It declares abort, the same backend-neutral approval lifecycle (its `approval` server request), and reverse Q&A (its `clarify` server request) — steer / permissions / compaction / fork / image input / native reuse are not wired and honestly report unsupported; a reverse request it has no card for (`sudo`, `secret`, `vault.*`, …) is answered immediately and left as a transcript notice instead of hanging the turn. A serve can be signed in to from an `agentred` (the daemon keeps that login in its `state.json`), but remote `agentred` execution is still refused: the runtime is not registered there, and a remote-bound hermes backend is blocked with a readable error instead of failing silently. See [§2.9](#29-hermes-backend-implemented-reference).
- **acp** — Agentre acts as an **ACP Client**: it spawns any external ACP Agent (`hermes acp`, `gemini --acp`, `npx -y @agentclientprotocol/codex-acp`, …) as a local CLI subprocess and speaks the Agent Client Protocol over stdio JSON-RPC (protocol **v1 only**, negotiated at initialize). It is not bound to an Agentre `LLMProvider` — the ACP agent owns provider/model/auth. The backend carries exactly two fields, `acpCommand` / `acpArgs`; capabilities beyond the 5-item matrix (`session/cancel`, `session/request_permission`, context-window reporting, negotiated image input, negotiated MCP HTTP injection) are honestly reported unsupported. Remote `agentred` execution works like claudecode/codex (it is a local CLI subprocess on the daemon box). See [§2.10](#210-acp-backend-implemented-reference).
...
- The `Capabilities()` implementation in `internal/pkg/agentruntime/runtimes/{builtin,claudecode,codex,piagent,openclaw,hermes,acp}/runtime.go`
...
| Capability (constant / wire string) | Sub-interface | builtin | claudecode | codex | piagent | openclaw | hermes | acp | Description |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `CapSteer` / `"steer"` | `Steerer` | ✅ | ✅ | ✅ | ✅ | ❌ | ❌ | ❌ | mid-turn injection of a user message; chat_svc generates the queuedID, and after the backend actually consumes it, it must emit `SteerConsumed`. OpenClaw does not claim a stable steer RPC |
| `CapCancelSteer` / `"cancel_steer"` | `SteerCanceler` | ✅ | ✅ | ❌ | ❌ | ❌ | ❌ | ❌ | retract after injection; once codex / piagent steer has entered the protocol it cannot be recalled, and OpenClaw does not expose steer |
...
| `CapAbort` / `"abort"` | `Aborter` | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ `session/cancel` | the "Stop" button; must be idempotent + must unblock all blocking I/O, otherwise the frontend will be stuck on "generating" forever. OpenClaw uses `chat.abort {sessionKey,runId}` |
| `CapSetPermission` / `"set_permission_mode"` | `PermissionModeSetter` | ❌ | ✅ mid-turn | ✅ launch only | ❌ | ❌ | ❌ | ❌ | switch permission mode at runtime; the codex protocol does not allow mid-turn switching, so it persists to DB and takes effect on the next spawn; piagent and OpenClaw do not expose permission mode meta |
| `CapAnswerUserAsk` / `"answer_user_ask"` | `AskAnswerSink` | ❌ | ✅ | ✅ | ❌ | ✅ | ✅ | ❌ | reverse-asking the user a question (single-select / multi-select / Other / password field); Skip must go through deny rather than an empty map, otherwise the turn silently hangs. OpenClaw requests the `operator.questions` scope at connect and maps Gateway `question.requested` / `question.resolve` onto this sink: each question answers as a string array, per-question `isOther` sets `AskQuestion.DisallowOther`, `isSecret` masks the input and the resolved answer is redacted to "answered, no content" (`agentruntime.RedactSecretAnswers`), and skip sends `{id, cancel:true}`. Hermes maps the v7 `clarify` server request onto this sink: single-select answers the chosen option's text, multi-select answers a JSON-array string, a question with no options answers free text, a batch aggregates by `qid`; single-question skip answers `""`, batch skip cancels the whole group; a question with options never offers Other |
| `CapToolPermission` / `"tool_permission_gate"` | `ToolPermissionSink` | ❌ | ✅ `can_use_tool` | ✅ `requestApproval` | ❌ | ❌ | ❌ | ✅ `session/request_permission` | allow/deny approval before tool execution + "Remember for session" (alwaysAllowSession). claudecode goes through the `can_use_tool` control_request and feeds DenyReason back to the LLM as a tool_result; codex goes through the app-server `requestApproval` protocol — it carries allow/deny + remember-for-session but has **no DenyReason feedback** (the deny-message param is ignored, the protocol has no deny field). piagent has no equivalent protocol; OpenClaw and Hermes route approval-shaped requests through `CapExecApproval` instead — `ToolPermissionSink` stays unimplemented for both |
| `CapExecApproval` / `"exec_approval"` | `ExecApprovalSink` | ❌ | ❌ | ❌ | ❌ | ✅ | ✅ | ❌ | backend-neutral approval lifecycle (the name predates the non-exec kinds it now also carries): decisions normalize to `allow-once` / `allow-session` / `allow-always` / `deny`, and a producer lists only the ones its backend accepts in `AllowedDecisions`. OpenClaw declares `caps:["approvals"]` at connect (otherwise the Gateway treats the connection as unable to deliver any of them) and covers three kinds — `exec` (command + the Gateway's `warningText`), `plugin` (plugin/tool/description), `system-agent` (description only — upstream system-agent requests carry title/description and no scope); an exec or plugin request's `scope` (`message-send` / `payment` / `external-post` / `standing-grant`) fills the card's action category and its scope fields — via `exec.approval.*` / `plugin.approval.*` / `openclaw.approval.*` plus the kind-agnostic `approval.resolve`/`approval.get`; it offers only allow-once/allow-always/deny (no per-session grant) and reconnect reconciles all three kinds by re-listing. Hermes maps its v7 `approval` server request (kind `hermes`) onto the same sink, offers whichever of once/session/always/deny the request's `choices` list, and converges a Gateway-withdrawn (`request.cancel`) or turn-end-unanswered card to expired while an answered-elsewhere card converges to resolved |
...
| Dimension | builtin | claudecode | codex | piagent | openclaw | hermes | acp | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Run shape | in-process (cago app/coding + LLMProvider) | CLI subprocess (stdout JSONL) | CLI subprocess (JSON-RPC over stdio app-server) | CLI subprocess (Pi RPC mode) | Gateway WebSocket RPC 4 | WebSocket JSON-RPC to an already-running `hermes serve` (Agentre never starts it) | CLI subprocess (stdio JSON-RPC, Agentre as ACP client, protocol v1 with negotiation) | determines the Prober / cli_path validation / transport ownership path |
| ProviderType binding | any LLM provider | Anthropic family (incl. gateway proxy) | OpenAI / Codex family | not bound to a provider; reads `~/.pi/agent` | Gateway-owned agent/model/auth | not bound: Hermes owns provider/model/auth; `ProviderTypeMatch` false | not bound: the ACP agent owns provider/model/auth; `ProviderTypeMatch` false | the entity `BackendKind.ProviderTypeMatch` implementation; piagent and OpenClaw always return false |
| Session mode | turn-scoped cago `Runner`, destroyed when the turn ends | long-lived subprocess + LRU cache reuse, reusing the session across turns | new process per turn, no local reuse | new Pi client per turn; Pi stores native JSONL in its configured/default root and Agentre resumes by the persisted native Session ID | stable `agentre:<backendID>:<sessionID>` key | one WebSocket per turn; Hermes owns session storage, Agentre resumes by the persisted native session id | long-lived subprocess + LRU cache reuse; one ACP session per chat session, resumed by the persisted ACP sessionId via `session/load` when negotiated (`session/new` + warn otherwise) | reuse means managing idle evict / abort unlocking / cross-turn state; OpenClaw returns the stable key as `ProviderSessionID` |
| Translator | pure function (cago events → sealed) | pure function + `task_aggregator` maintaining the Subagent list across turns | pure function | pure RPC mapper + turn-local subagent tracker in the `Run` drain loop | run/session/seq filter + payload translator | pure function (gateway frame → sealed event) | pure function (`SessionUpdate` union → sealed events); tool-call title/status aggregation stays in the `Run` drain loop | state aggregation stays at the drain boundary and the translator itself must remain independently table-testable; OpenClaw reconnect aggregation belongs in the runtime, not service/repository |
| Plan source | does not emit | `TodoWrite` tool inline (canonical) + `Task*` incremental aggregation (PlanUpdated snapshot) | `turn/plan/updated` (Steps) + `item/plan/delta` (Text) merged into a single PlanUpdated | does not emit | does not emit | `todo.updated` → PlanUpdated | `plan` (Entries) → PlanUpdated | what downstream chat_svc sees is the same sealed `agentruntime.PlanUpdated`; OpenClaw plan capability is not claimed |
| Reverse Q&A | not supported | control_request `can_use_tool` + `AskUserQuestion` tool | app-server `item/tool/requestUserInput` JSON-RPC | not supported | Gateway `question.requested` / `question.resolve` RPC (requires the `operator.questions` scope) | v7 `clarify` server→client request (requires `client.capabilities {server_requests:true}`, §2.9) | `session/request_permission` JSON-RPC (select by option **kind**; no deny-reason feedback channel) | claudecode uses the question text as the key, codex uses the question ID as the key, OpenClaw uses the Gateway `questionId`, Hermes uses `qid`; see [§2.4 D](#d-askanswersink--reverse-asking-the-user-a-question) for the full answer-shape comparison |
...
| Remote daemon | ✅ | ✅ | ✅ | ✅ | ❌ | ❌ (refused: a hermes backend has no LAN `agentred` dispatch path) | ✅ | the runtime must not depend on desktop-only services (chat_repo / GUI); OpenClaw and Hermes runtimes are not registered on `agentred` (their device-local credentials are enrolled there, §2.8 / §2.9) |
| New-session provider selection | ✅ any provider | ✅ Anthropic family | ✅ OpenAI / Codex family | ✅ Anthropic + OpenAI family | ❌ never rendered | ❌ never rendered | ❌ never rendered | a new-session (`sessionId===0`) composer `ProviderPill` lists only the LLM providers compatible with the backend, gated by `ProviderTypeMatch` (builtin→any, claudecode→anthropic, codex→openai-response, piagent→anthropic/openai-chat/openai-response, openclaw→none — the pill is not rendered for it). Picking one persists `chat_sessions.provider_key`; each turn resolves `effectiveProviderKey = firstNonEmpty(sess.ProviderKey, be.LLMProviderKey)` → that provider's default model. Existing sessions render no switcher — see [§2.4 J](#j-per-turn-model-provider-default-only-no-session-level-override) |
...
| Field | builtin | claudecode | codex | piagent | openclaw | hermes | acp | Field meaning |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
| 2. Database migration | `migrations/YYYYMMDDNNNN_*.go` + append to the end of `migrationList()` | Only for a column **shared across backend types**; settings only your type owns need no migration (see 2.1) |
...
- Settings that **only your type owns** get a `gorm:"-"` field on `AgentBackend` plus a matching `omitempty` key in `backendConfig` (`config.go`). They persist inside the single `config_json` column, so adding a backend costs no migration and no new column — `agent_backends` is shared by every type, and a column that only one type ever writes states nothing that `BackendKind` does not already own. A migration is only needed for a setting that **several** types read (`cli_path`, `reasoning_effort`, `env_json` are the existing ones).

  The trade-off is that these settings cannot be query predicates. That has never been needed: they are read on one path only — resolve the backend, then start the subprocess.
...
> A backend that drives an **already-running service** (openclaw / hermes) skips the CLI-probing and env-wiring parts of this section entirely: it has no local process, and its Prober reuses the protocol dial and stops at the handshake. See [§2.9](#29-hermes-backend-implemented-reference).
...
| `CapReportContextWindow` | emit `ContextWindowUpdated` | the runtime can probe the model's actual window (codex has it natively in the protocol; claudecode falls back via `llmcatalog.Lookup(model)`; piagent exclusively reads Pi RPC metadata (`get_state.model.contextWindow` at startup plus `get_session_stats.contextUsage.contextWindow` refreshes), carrying the live value on usage snapshots so custom-provider model metadata remains authoritative) |
...
| `CapMCPTools` | `RunRequest.MCPServers` (no sub-interface) | the runtime consumes MCP tool servers injected at launch; the org/subagent MCP tools gate on this cap — see the matrix above |
...
6. **runtime writes back to the backend**:
   ...
   - **openclaw**: sends `question.resolve {id, answers:{answers:{qid:[...]}}}` — one string array per question ID (`internal/pkg/agentruntime/runtimes/openclaw/questions.go`). A secret question's raw text goes to the Gateway only; before the event reaches chat_svc it is already redacted by `agentruntime.RedactSecretAnswers`, so `UserAskResolved.Answers` never carries the content.
   - **hermes**: replies to the v7 `clarify` server request on the same request id (`internal/pkg/agentruntime/runtimes/hermes/clarify.go`) — single-select answers the chosen option's text, multi-select answers a JSON-array string, a question with no options answers the free text verbatim, and a batch of questions aggregates the reply by `qid`.
...
7. **Skipped semantics**: must let the LLM gracefully see the refusal signal; **do not** allow an empty map (which silently hangs the turn, hapi gotcha #4):
   ...
   - openclaw: sends `question.resolve {id, cancel:true}`.
   - hermes: a single question answers `""`; a batch cancels the whole group (no `answers` field sent at all).
...
- AskQuestion.ID is backend-private (codex uses it as the key; claudecode uses the question text as the key; OpenClaw uses the Gateway `questionId`; Hermes uses `qid`) — the translator must preserve it and not drop it.
- AskQuestion.DisallowOther is decided per question by the backend, not globally: claudecode/codex never set it (unchanged behavior); OpenClaw sets it to `!isOther`; Hermes sets it whenever the question carries options, because Hermes rejects free text there.
...
- A credential never rides `RunParams`. A backend whose credential lives on the device it is bound to enrolls it on that device through the device credential RPCs (status, OpenClaw token set, Hermes providers / login / logout, connection test — `RPC_METHOD_BACKEND_CREDENTIAL_STATUS` … `RPC_METHOD_BACKEND_CONNECTION_TEST`), which `agentred` answers from `state.json` without registering the runtime. Anything the daemon's credential handlers need must therefore sit in a side-effect-free package (`internal/pkg/backendcred`, `runtimes/hermes/hermesauth`, `runtimes/hermes/hermesgateway`, `internal/pkg/openclawgateway`) — importing a runtime package would register the runtime through its `init()`. OpenClaw and Hermes are enrolled this way but not blank-imported yet; `runtime_imports_test.go` pins that neither is registered.
...
2. **Secret boundary** — the credential logic lives in `internal/pkg/backendcred`: the token slot is `agentre.openclaw.backend.<syncID>.token` (the account-wide `sync_id`, so every device names the same backend the same way) and the Ed25519 identity seed has its own per-device slot, generated locally and never sent anywhere. The desktop backs the slots with `internal/pkg/keychain`; `agentred` backs them with `state.json`. DTOs expose `hasToken` only. OpenClaw-specific create/update/test Wails methods accept a transient token argument; the editor never receives the saved token.
3. **Protocol client** — `internal/pkg/openclawgateway` implements challenge/connect, exact protocol-4 negotiation, required operator scopes (`operator.read` / `operator.write` / `operator.approvals` / `operator.questions` — a device paired before `operator.questions` existed gets `PAIRING_REQUIRED` with reason `scope-upgrade` until the operator re-approves it), req/res routing, event sequence gaps, timeouts, reconnect, feature validation, and read-only agent/model discovery. Connect also declares `caps:["approvals"]`; without it the Gateway's `canDeliverApprovals` treats the connection as unable to answer any of the three approval kinds below and either rejects the exec outright (no other approver) or routes it to a different approver, invisible to Agentre.
4. **Runtime** — `internal/pkg/agentruntime/runtimes/openclaw` submits the turn with the UUID `idempotencyKey` required by the official schema and a stable `agentre:<backendID>:<chatSessionID>` session key. It submits via `chat.send` by default — after first subscribing to `sessions.messages` on that session key, which is what makes the Gateway record this device as the run's approval reviewer — and falls back to `agent` only when an admin-scoped model override is configured or the Gateway does not advertise `chat.send` / session-subscribe (`runtime.go`, around the `useChatSend` computation); `agent` broadcasts turn events to every subscriber but is not bound to a reviewer device, so approvals raised by an `agent`-submitted turn are still visible, just not reviewer-scoped to this device. (Earlier drafts of this doc said OpenClaw always submits via `agent` — that was never what the code did.) Reconnect/gap reconciliation uses `exec.approval.list` / `plugin.approval.list` / `openclaw.approval.list` / `question.list` followed by `agent.wait`; it never blindly resends the message. Abort sends exactly `chat.abort {sessionKey,runId}` and converges through that stable identity plus a one-call guard because the official closed abort schema rejects an added idempotency field.
5. **Approvals** — `CapExecApproval` / `ExecApprovalSink` is distinct from `CapToolPermission` and, as of the 2026-09-17 approval-and-question-prompts round, backend-neutral: it now covers all three kinds `caps:["approvals"]` makes deliverable — `exec` (command + `warningText`, `exec.approval.*`), `plugin` (plugin/tool/description, `plugin.approval.list` + `plugin.approval.resolve`), and `system-agent` (description, `openclaw.approval.list` + the kind-agnostic `approval.resolve` / `approval.get` — Gateway has no per-kind system-agent resolve RPC; upstream system-agent requests carry only title/description). The Gateway's `ApprovalScope` rides on exec and plugin requests only: `message-send` → message category + target/recipients + recipient count, `payment` → amount + currency and payee, `external-post` → publish target + visibility, `standing-grant` → automation + its command (`approvals.go`'s `applyApprovalScope`). Requested/resolved events persist the same approval block, tagged by `ApprovalKind`. Resolution sends exactly `{id, decision}` to `exec.approval.resolve` / `plugin.approval.resolve`, and `{id, kind, decision}` to `approval.resolve`, because those official closed schemas reject extra idempotency fields and the generic one requires `kind`; duplicate/racing resolutions converge by approval ID and terminal state. OpenClaw offers only `allow-once` / `allow-always` / `deny` — it has no per-session grant, so `allow-session` never appears in `AllowedDecisions`. AgentRE trusts Gateway `allowedDecisions`, never reconstructs a Node `systemRunPlan`, and never treats approval terminal as execution terminal; a turn ending expires every approval still unanswered, and reconnect reconciles all three kinds by re-listing.
6. **Questions** — `CapAnswerUserAsk` / `AskAnswerSink` maps OpenClaw's `question.*` protocol (`internal/pkg/agentruntime/runtimes/openclaw/questions.go`): a `question.requested` record for the turn's session key becomes an `AskQuestion` card, with each question's `isOther` setting `DisallowOther` and `isSecret` masking the input. `SubmitAnswer` sends `question.resolve {id, answers:{answers:{qid:[...]}}}` — one string array per question — or `{id, cancel:true}` to skip. A secret question's raw text goes to the Gateway only: the terminal event, transcript, persistence, and `openclawgateway`'s debug frame logging (`redactQuestionAnswers`) all see it as answered without content (`agentruntime.RedactSecretAnswers`). Withdrawal, expiry, or an answer landing on another surface converge to skipped/answered via `question.resolved`; reconnect reconciles pending questions the same way as approvals, gated by whether the Gateway broadcasts `question.list`.
7. **Yielded turns** — a `chat.final` (or lifecycle-end) payload carrying `yielded:true` does not end the turn: `internal/pkg/agentruntime/runtimes/openclaw/active.go`'s `activeTurn` records the run as yielded and keeps the turn open, waiting for the same session's next run while filtering out any late frames still tagged with the yielded run's own id. A follow-up run's non-yielded terminal ends the turn normally; a follow-up that itself yields keeps the turn open again. Stopping aborts whichever run is currently active; while waiting between runs no run can be named yet, so Stop sends `chat.abort` for the session without a `runId` (aborting whatever the session is running, including a follow-up whose first frame has not arrived) and ends the turn as aborted at once. If the connection drops while a follow-up is unconfirmed and reconnect cannot establish its state, the turn ends with the readable `code.OpenClawYieldFollowUpUnconfirmed` error instead of waiting forever.
8. **Frontend** — settings use `OpenClawBackendFields`; transcript rendering uses the backend-neutral approval card (`frontend/packages/agentre-ui/src/transcript/openclaw-exec-approval/` — the directory/component name predates the generalization and now also renders Hermes' `hermes` kind, see [§2.9](#29-hermes-backend-implemented-reference)) and the shared question card. Both use production shadcn/design-system primitives and bilingual i18n.
9. **Remote boundary** — `agentred` answers the device credential RPCs for a backend bound to it: token status, save/clear (`OpenClawTokenSetRequest`, keyed by `sync_id`), and a connection test that probes the Gateway with the stored token (or a one-time draft token that is never stored) and the daemon's own identity, returning the desktop's result codes (`AUTH_FAILED`, `OPENCLAW_CONNECTION_FAILED`, …). Responses report only whether a token is saved. The runtime itself is not registered on the daemon; `RunParams` contains no OpenClaw secret and remote execution reports capability unavailable. Approval and question event codecs are wire-compatible.

### 2.9 Hermes backend (implemented reference)

Hermes is the reference for a backend whose transport is a **separately started service** — Agentre never spawns or stops it — and whose credentials cannot live in entity DTOs:

1. **Entity/migration** — `TypeHermes` owns exactly one transport field, `HermesURL` (canonical `http(s)://host:port`; the runtime swaps the scheme for WS). It deliberately accepts no `cli_path` / interpreter knobs, because there is no local process to launch. `hermesAuthProvider` / `hermesUserId` are non-sensitive display fields in `config_json`; no token/ciphertext column exists.
2. **Secret boundary** — the refresh token is stored under an account derived from the normalized URL (`backendcred.HermesAccount`), so one serve shares one credential on a device and changing the URL forces a new sign-in. The desktop stores it through `internal/pkg/keychain`; `agentred` stores it, with the display identity, in `state.json`. The password is used once and never persisted or logged; deleting the backend deletes the credential.
3. **Protocol client** — the side-effect-free `runtimes/hermes/hermesgateway` owns the WebSocket dial, the loopback token handshake, the gated bearer → one-shot 30-second `ws-ticket` handshake, and failure classification (`ErrGatewayUnreachable` / `ErrGatewayUnauthorized` / …); `runtimes/hermes/hermesauth` implements the PKCE native flow (authorize → password-login → native token → refresh, sentinels `ErrLoginRequired` / `ErrLoginExpired` / …) and never opens a browser or a local listener. The runtime package builds its JSON-RPC session (`client.go` / `frame.go`) on `hermesgateway.Dial` and re-exports both packages' sentinels.
4. **Runtime** — one WebSocket per turn; the connection is the per-session ownership unit the fence guards. Right after `gateway.ready`, the session declares `client.capabilities {server_requests: true}` once (`client.go`'s `defaultSessionFactory` → `declareServerRequests`); Hermes only sends `approval` / `clarify` (or any other server→client request) to a connection that declared this, and otherwise treats every such request as unanswerable and withdraws it — the failure mode this round fixed (spec Problems 1–2). The session is created-or-resumed and the submitted session id returns as `RunResult.ProviderSessionID` for the next turn. Abort sends `session.interrupt`. `CapAbort`, `CapExecApproval` and `CapAnswerUserAsk` are declared; everything else honestly reports unsupported.
5. **Translator** — a pure function from gateway event frames (`message.start|delta|interim|complete`, `thinking.delta`, `reasoning.delta`, `tool.start|generating|complete`, `todo.updated` → `PlanUpdated`, `session.usage`) to sealed events; every unknown event frame produces no events (the raw-frame sink logs them), so a frame we do not understand can never wedge the turn. The v7 `approval` and `clarify` messages are not gateway events — they are id-addressed server→client *requests*, routed by `frame.go`'s `routedServerRequests` and synthesized into the ordered stream as `EventServerRequest`, then answered asynchronously by the turn (bullets 6–7 below). This replaced the old v6 event-shaped `approval.request` / `clarify.request` no-op path, which this doc previously (incorrectly) described here — that path never produced a usable card and has been removed from the code entirely.
6. **Approvals** — the v7 `approval` server→client request becomes an `ExecApprovalRequested{ApprovalKind: "hermes"}` card carrying the command, description and tool name Hermes already redacted (`approvals.go`). Its `choices` (`once` / `session` / `always` / `deny`, gated by `allow_session` / `allow_permanent`) map onto normalized decisions (`allow-once` / `allow-session` / `allow-always` / `deny`) in `AllowedDecisions`, in presentation order — Hermes is the only backend that can offer `allow-session`. `ResolveExecApproval` answers `{choice: ...}` on the same request id, serialized so a race or a repeat resolves the Gateway exactly once. `request.cancel` converges the card: reason `resolved` → resolved (answered on another surface), every other reason (timeout / interrupted / shutdown / session_closed) → expired; any card still unanswered when the turn ends also expires.
7. **Questions** — the v7 `clarify` server→client request becomes an `AskQuestion` card (`clarify.go`); a question that carries options never offers Other, because Hermes rejects free text on those. `SubmitAnswer` replies per Hermes' own shape: a single-select question answers the chosen option's text, a multi-select answers a JSON-encoded array of the chosen texts, a question with no options answers the free-text input verbatim, and a batch of questions aggregates the reply by `qid`. Skipping a single question answers an empty string; skipping a batch cancels the whole group (sends no `answers` at all). `request.cancel` and an unanswered card at turn end both converge to skipped, matching the Question cards spec (unlike approvals, clarify withdrawals never resolve to "answered elsewhere").
8. **Unsupported requests** — `sudo`, `secret`, `vault.unlock_prompt`, `vault.save_login`, `vault.code`, `terminal.read`, `preview.read`, `window.read`, `preview.act` and `tour` are answered immediately on the read loop with the contract's `{"value":""}` (`frame.go`'s `unsupportedServerRequests`) — their params are never unmarshaled into an event or a log line. Each one also surfaces as `agentruntime.UnsupportedRequestNotice{Purpose}` (method name only, mapped onto its purpose by that same table), which `handlers.UnsupportedRequestNoticeHandler` turns into a transcript notice naming only the purpose category (e.g. "sudo password", "read terminal output") — never the raw protocol method or params. Any other, unrecognized method is answered `-32601 method not found` on the same read loop and still leaves a notice, with the `unknown` purpose ("an unknown action").
9. **Prober / Test Connection** — `hermesgateway.Probe` (re-exported as `hermes.Probe`) reuses the same dial path and stops after `gateway.ready`; it does not run a turn. A gated serve answers `GET /` with a 302 to its login page, so "needs login" is detected by "the page carries no session token" — **not** by a 401 and **not** by any marker element.
10. **Frontend** — settings render a single Server URL plus the auth block (provider list, sign in / sign out, signed-in user) with bilingual i18n. Providers that do not support password sign-in are listed but disabled with readable copy; OAuth is not implemented.
11. **Remote boundary** — `agentred` answers the device credential RPCs for a serve: list providers, log in (the password is used for that request only), log out, login status, and a connection test through `hermesgateway.Probe` with the daemon's stored login, returning the desktop's result codes (`HERMES_LOGIN_REQUIRED` / `HERMES_LOGIN_EXPIRED` / `HERMES_UNREACHABLE` / …). Chat turns are still never dispatched to `agentred`, where the runtime is not registered: `exec_target_svc` blocks a remote-bound hermes backend, and `chat_svc` returns a hint error (`ChatBackendHintRemoteHermes` / `ChatBackendRemoteHermesUnavailable`) instead of failing silently.

### 2.10 ACP backend (implemented reference)

ACP is the reference for a backend where Agentre is the **protocol client** and the agent process is the server: any external ACP Agent is spawned as a local CLI subprocess and driven over stdio JSON-RPC. Unlike hermes §2.9, Agentre owns the process lifecycle.

1. **Entity/migration** — `TypeACP` owns exactly two launch fields, `ACPCommand` (executable) and `ACPArgs` (argv tail), stored in `config_json`; there is **no migration** because `config_json` already exists. Both reject `\n` / `\r` / NUL to keep probe log lines single-line. No `cli_path`, no provider binding (`ProviderTypeMatch` is false, `RequiresProviderModel` false), no permission-mode / model / routes / sandbox knobs — the ACP agent owns all of that. The `Check` guard rejects acp fields on any other backend type.
2. **Secret boundary** — none. The backend entity carries no keys or tokens; the spawned ACP agent brings its own login state (e.g. its own `~/.gemini`, claude-code config). `BuildACPEnv` passes through only `env_json` entries and injects no gateway variables.
3. **Protocol client** — `runtimes/acp/client.go` owns the version negotiation: `initialize` advertises `protocolVersion: 1` (the highest this build speaks) plus `clientInfo: agentre`, and does **not** declare fs or terminal read/write. If the agent answers a different version, the handshake fails with a readable error (`agent %q requires ACP protocol version %d, but this build supports only v1`) — v2 is explicitly rejected, never silently coerced. When v2 support lands it goes in as an additional version adapter beside the v1 one, all inside this file. The negotiated capabilities (`loadSession` / `promptCapabilities.image` / `mcpCapabilities.http`, agent info, auth methods) are recorded once here and consulted everywhere else.
4. **Runtime** — one long-lived subprocess per chat session, reused across turns through the `CLISessionPool` with an identity fingerprint over command/args/cwd/env plus the MCP server list (a changed MCP manifest evicts the process). First turn: `initialize` → `session/new`, and the returned ACP sessionId is persisted as `RunResult.ProviderSessionID`. Pool miss on a later turn: if the agent negotiated `loadSession`, resume via `session/load` (re-sending the current MCP list); otherwise fall back to `session/new` with a warning — the conversation context does not carry over. `FreshSession` forces `session/new`. Abort sends `session/cancel` with a grace period, then kills the process tree; any pending permission request is answered `cancelled` first (protocol requirement).
5. **Translator** — a pure function from `SessionUpdate` variants to sealed events (`user_message`/`agent_message` chunk → text deltas, `agent_thought_chunk` → thinking deltas, `tool_call`/`tool_call_update` → ToolCall/ToolResult, `plan` → PlanUpdated, `usage` → UsageUpdate). Unknown update kinds produce no events (the raw-frame sink logs them), so a future protocol addition can never wedge the turn. Tool display names resolve title → kind → `other`; `rawInput` is re-marshaled (the SDK decodes it, so byte-exact passthrough is not reachable).
6. **Image / MCP are negotiated, not assumed** — if the user sends an image and the agent did not negotiate `promptCapabilities.image`, the turn fails with a readable error (`errImageNotNegotiated`); images are never silently dropped. If MCP servers are configured but the agent did not negotiate `mcpCapabilities.http`, the list is not injected, a warning is logged, and the turn runs normally (no user-visible notice). Note that remote dispatch to `agentred` never injects MCP servers in any case — `AgentBackendHasCapability` returns false for remote backends — which is a pre-existing cross-backend fact, not acp-specific.
7. **Reverse approval** — `session/request_permission` is served by the `acpsdk.Client` implementation: the request is surfaced as `ToolPermissionRequest` (RequestID is a synthesized `acp-perm-N` handle because the SDK does not expose the JSON-RPC id) and the turn blocks until the user answers. Options are selected by **kind** (allow_once/allow_always/reject_once/reject_always), never by re-using an optionId; a missing kind answers `cancelled` plus a warning rather than fabricating an id. Deny reasons have no protocol channel back, so they are logged and dropped. Duplicate or unknown request ids are idempotent no-ops.
8. **Prober / Test Connection** — `acp.Probe` spawns the command, completes `initialize`, and stops: no `session/new`, no prompt. The success message names the agent, the protocol line (`ACP v1`), the three negotiated capabilities (`image` / `mcp-http` / `loadSession`), and the agent's advertised auth methods.
9. **Remote boundary** — dispatchable to `agentred` like any local CLI subprocess: `exec_target_svc` treats it as chattable on both local and remote, `chat_svc` resolves it without touching providers/gateways, and the daemon registers the runtime via its blank import. The MCP-injection caveat above is the only behavioral difference from local. They are account-level identity: there is deliberately **no per-device override** for them (unlike `cli_path`, acp has no "known CLI" whose path could differ per machine).

   There are two dispatch shapes, and they deliberately do not share the launch-body path:

   - **Direct desktop call** — the local Create/Update DTOs produce the entity, the daemon protobuf `AgentBackend` message carries `acp_command = 28` / `acp_args = 29`, and `protowire.BackendFromProto` rebuilds the row on the daemon (a missing mapping surfaces as `backend has no acpCommand configured` on the remote box). This is the original path and still how a paired desktop dispatches.
   - **Browser / cloud dispatch** — sends only `{type, sync_id}`. The daemon resolves the whole `syncwire.AgentBackendConfig` for that sync ID from the `backends` array in the device-JWT `GET /v1/engine/snapshot` response and applies it (as one unit) in `RuntimeHandlers.Run` before runtime selection. `acp_args` is arbitrary argv and may contain credentials, so it must **never** be placed in a browser dispatch-plan response or copied through a browser API — the browser only names the backend; the daemon already holds the config. An absent snapshot entry leaves a directly-supplied config untouched, so the two paths coexist. See `internal/daemon/enginesnapshot` (`ResolveBackendConfig`) and the daemon-side `BackendConfigForSyncID` dep.
10. **Not declared** — steer / cancel-steer / drain-steer, permission-mode switching, ask-user (single-select Q&A — distinct from tool permission), exec approval, fork-session, compact, skills, subagent events, autonomous turns: the ACP v1 protocol has no channel for these, so they honestly report `ErrUnsupported`.
11. **`agrctl acp` — the reverse direction (Agentre as the driven ACP agent)** — the same `agrctl` companion CLI can run as an ACP v1 agent itself (`internal/cli/acpcmd`, `agrctl acp --agent <name>`), so an `acp` backend whose `acpCommand` is the installed `agrctl` and `acpArgs` is `["acp", "--agent", "<name>"]` drives another agent configured in this desktop. It advertises `loadSession: false` and `promptCapabilities.image: true` (without the latter the Agentre client never sends images), creates one Agentre session per ACP `session/new` via `POST /ctl/v1/sessions`, and for each `session/prompt` opens that session's SSE stream (`GET /ctl/v1/stream`) **before** dispatching `POST /ctl/v1/send` — the ordering is what prevents losing the first chunk. It consumes the desktop `chat_svc` fan-out events (`NewFanoutEmitter` wrapped outside the coalescing emitter) and translates `chunk` → `agent_message_chunk`, `thinking` → `agent_thought_chunk`, `tool_use` → `tool_call`, `tool_result` → `tool_call_update`; `done` / `error` / `aborted` end the turn (`end_turn` / error / `cancelled`). `session/cancel` posts `/ctl/v1/stop` and returns `cancelled` immediately. **Not translated**: `ask_user_question` / `tool_permission_request` / `plan_update` / `subagent_*` are skipped and never forwarded as ACP `session/request_permission` — approvals and questions stay in the desktop UI; the other event kinds are simply dropped, which must never wedge the turn. The recursion hazard (targeting an `agrctl acp` whose own target uses the same `acp` backend) is **documented only**, not detected in code.
