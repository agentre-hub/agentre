...
| 4 | 行首固定一个 14px 槽位，放**分组没说的那一维**：分组＝项目 → agent 头像；分组＝Agent → **项目字形**（项目自己的图标 + 项目色，与项目组头上那一枚同源）。 | 同位置同尺寸，切分组时列表不跳动。自由会话在「按 Agent」下槽位保留、字形退成「随手对话」的中性字形，行的左缘不参差。拒绝「按 Agent 时不渲染字形」（左缘参差）；拒绝把另一维放行尾（与 trailing 状态文案争位）；拒绝通用文件夹字形（用户裁决 2026-08-18，见「实现期补充」第 10 条）。 |
| 5 | 「按时间」用**两行行式**：第一行标题 + 状态，第二行 `〔agent 头像〕agent · 〔项目字形〕项目`（两维各自带它在别档里的那一枚字形）。 | 这一档没有组头吃掉纵向空间，两行放得下；而它恰恰是唯一需要同时给出两维的一档。拒绝单行塞两个字形（两个无文字图标读不出来）。 |
...
决策 4 要求「按 Agent」分组时行首放项目字形，决策 5 要求时间轴第二行给
...
| Agent | `session.agentID` | agent 组头（pin / ＋ / 折叠） | 项目字形：项目图标 + 项目色（自由会话退「随手对话」中性字形） |
| 时间 | 无分组 | 无 | 两行行式，两维都给 |
...
8. **F3 那笔「代价」已经还掉了**（2026-08-17）。删掉 1 秒轮询时接受的那句「另一台
   设备同步过来的项目要等一次别的交互才出现」不是可接受的现状 —— `make e2e` 的
   `sync-client.spec.ts` 正是这么红的：项目已经落库，界面上就是不出现。修在producer：
   `sync_svc` 在一轮下行**真的落地了东西**时发 `sync:applied`（空转的轮次不发），
   前端 `SyncAppliedHost` 收到就走 `reloadSidebarSources()`。项目树从此有了推送通道，
   轮询也不必回来。
9. **「随手对话」不给直达快捷键**（用户裁决 2026-08-17）。决策 6 的两条路（组头 `＋`、
   命令面板项目选择器的「无项目」）够用。mockup 的 `09-free.png` 说明里写的是「三条路
   缺一不可」，第三条就是这个快捷键 —— 那张图与规格从一开始就分了岔，现在按规格收口。
10. **行里的项目那一维画项目自己的图标，不画通用文件夹**（用户裁决 2026-08-18）。
   一枚通用文件夹只带得动「这是个项目」，带不动「是哪个项目」—— 三个项目在行首长
   一个样，只有色相不同；而项目在组头上早就有身份（自己选的图标 + 项目色，没选图标
   时是名字首字母）。行里换成同一枚字形之后，「按 Agent」的行首、「按时间」的第二行、
   项目轴的组头三处是同一个项目的三种尺寸。
   落地为一个组件 `session-index/project-glyph.tsx`，三处共用 —— 此前组头与行各画各的，
   正是两者长不一样的由来。自由会话（`project === null`）在同一个组件里退成
   「随手对话」组头那枚 `MessagesSquare` + 中性面，槽位与置灰的口径不变（决策 4/7）。
   连带把 `projectColorOf` / `projectNameOf` 两个取值函数并成一个 `projectInfoOf`：
   画一枚字形要的三件事（名字 / 颜色 / 图标）只在一处一起用，拆开取只会让「同一个
   项目在两处长得不一样」重新有机可乘。
