...
**桌面端执行者：** 经现有服务层完成读写，不绕过服务层的校验，也不绕过同步通知（`sync_svc.Notify*`）。提供方的 `--api-key` 省略表示保留原值；API key 不能置空，显式空串判为用法错误（退出码 2），与桌面端设置界面一致（`llm_provider_svc` 的 Update 把空值当作保留，用户 2026-09-23 决定不新增清空能力）。OpenClaw 的 `--token` 写空串表示清空，由服务层路由到后端绑定设备的钥匙串，设备离线时报错，与编辑器的现有行为一致。
