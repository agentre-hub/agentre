...
6. **老对端不丢配置。** 不带新 wire 字段的旧桌面端向新 agentred 发起的轮次，仍按 backend 负载里的力度跑；新字段缺省不得被解读成「用户选了默认档」。
...
| 5 | daemon 取值优先级：**run 参数非空 > backend 负载**，缺省不视为「用户选了默认」 | 硬不变量 6。旧桌面端不带新字段，若把缺省解读为空档，它们的后端配置会在升级 agentred 后集体失效。空档本身经桌面端合成后仍会以「后端配置值」的形态出现在 backend 负载里，两者不冲突 |
