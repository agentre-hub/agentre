...
| Entry | Contents |
| --- | --- |
| `@agentre-ai/agentre-wire` | frame types + `decodeXxx` / `encodeXxx` + protocol constants |
...
| `src/*.gen.ts` | generated | every frame type, codec and protocol constant |
...
**Staleness and coverage are both caught mechanically**, by three always-on guards in the wire package (see [testing.md](./testing.md#guard-tests)): the committed `*.gen.ts` and `fixtures/*.json` must be exactly what the generators write today, and the generator's type/constant list must cover everything the wire package exports. Adding a Go field, or a whole Go struct, turns the build red instead of silently leaving the browser decoding an obsolete shape. The guards are self-contained within this repository and never read a sibling checkout.
