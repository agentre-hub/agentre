...
The desktop app owns the local database and creates/reuses the `chat_sessions` row through `chat_svc`. When a turn starts, runtime selection decides whether execution is local or proxied through `remote.Runtime` to an `agentred` daemon. The remote daemon executes the turn and reports runtime state; it does not own the desktop session lifecycle. It does, however, own a daemon-side session identity row and the durable record of that turn's event stream: `agentred` journals every notification to its own SQLite store under a gap-free monotonic `seq` before pushing it, so the record survives a dropped connection even though session identity, sidebar state, read state, and issue linkage stay authoritative on the desktop. The journal remains available for catch-up until the owning session is deleted. `runtime.session.delete` removes the daemon-side session row and its complete notification journal, and closes the backend session; deletion is scoped by the authenticated peer fingerprint so another desktop's equal numeric session id is untouched.
