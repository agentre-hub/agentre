...
Remote execution does not move desktop session creation to `agentred`.

The desktop app owns the local database and creates/reuses the `chat_sessions` row through `chat_svc`. When a turn starts, runtime selection decides whether execution is local or proxied through `remote.Runtime` to an `agentred` daemon. The remote daemon executes the turn and reports runtime state; it does not own the desktop session lifecycle. It does, however, own a daemon-side session identity row and the durable transcript of that turn: `agentred` persists it to its own SQLite store in the same message-row + block-row shape as the desktop and numbers the frames it serves with a gap-free monotonic per-conversation `seq`, so the record survives a dropped connection even though session identity, sidebar state, read state, and issue linkage stay authoritative on the desktop. The transcript remains available for catch-up until the owning session is deleted. `runtime.session.delete` removes the daemon-side session row together with its complete transcript (block rows, frame-number ledger, message rows), and closes the backend session; deletion is scoped by the authenticated peer fingerprint — naming another peer's conversation is an account-level capability that pairing alone never grants — so a peer cannot delete a conversation recorded under someone else.

A session's push target on `agentred` is the connection that started it — identified by (device fingerprint, conversation id) — not "any connection from that device": a desktop can hold several authenticated connections at once (the one running the session, plus others such as a device-status heartbeat), and only the owning connection receives that session's notifications. If that connection dies, `agentred` does not clean up the session or its subprocess; it keeps persisting the transcript and suspends pushing until a same-fingerprint connection explicitly takes the session back over via the `runtime.session.attach` RPC.

To make this recoverable, `chat_sessions` carries three columns describing execution position: `exec_device_id` (the paired daemon), `exec_device_fingerprint` (that daemon's instance identity), and `event_cursor` (the last `agentred` `seq` the desktop has consumed). The cursor is only valid for the daemon instance that issued it — reinstalling, migrating, or wiping a daemon's data directory changes its fingerprint, and a cursor recorded against the old fingerprint is treated as invalid rather than replayed against the new instance's frame numbering, so the session is instead treated as unrecoverable. On reconnect, for each session it still holds locally, `remote.Runtime` re-attaches ownership, validates and re-syncs the cursor, pulls the frames numbered after it, and queries pending decisions, replaying everything through the same handlers used for live traffic (`internal/daemon/handlers/session_catchup.go`).

The cursor otherwise only ever moves forward, with two exceptions — both of them cases where it points at a position the daemon can no longer serve, and in both, not resetting it freezes the session with no error and no reported gap. If attach hands back a high-water mark *below* the recorded cursor, that daemon's frame numbering went backwards (its database was restored or truncated while its instance identity survived), and catch-up restarts from zero, invalidating the stored cursor too. If an incremental pull reports an `oldestSeq` *above* `cursor + 1`, that range of the daemon's transcript is missing some other way (its database was pruned by hand, or by a future cleanup pass — `agentred` itself no longer reclaims anything); the cursor is reset to `oldestSeq - 1`, the missing tail is accepted as gone, and a warning is logged. The reset happens before that page is replayed, or its first row would itself be dropped as a seq gap.

Catching up after an app restart starts from the same three columns rather than from in-process state, because there is no in-process state left: `chat_svc.CatchUpRemoteSessions` runs at startup, reads every session carrying an `exec_device_id`, groups them by paired daemon, and per daemon asks the session-listing RPC for the lifecycle state, waiting-for-input flag and frame high-water mark of each. That answer decides who needs work — a session that is idle and already at the daemon's high-water mark costs no further round trip, one that is still running is re-attached even when it has produced nothing yet (attach is what claims the push stream), and one the daemon reports as interrupted has its pre-crash history pulled without an attach and is then closed as interrupted rather than as a disconnect.
