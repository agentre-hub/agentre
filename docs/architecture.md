...
frontend/                      (React 19 + TS + Vite + Tailwind; wailsjs/ is wails-generated, gitignored)
  ...
  packages/agentre-wire/       (@agentre-ai/agentre-wire —— zero-dependency wire-protocol golden samples, generated by the
                                Go marshaler in internal/pkg/agentruntime/runtimes/remote/wire. See frontend.md)
...
`frontend/packages/agentre-wire` is the second shared package and deliberately a separate one: it carries the wire-protocol **golden samples** — pure data with no runtime dependency and no build step — so it stays out of `agentre-ui`, whose React / tiptap peer dependencies have nothing to do with a JSON fixture. Its rules live in [`frontend.md`](frontend.md#shared-wire-fixtures-package-agentre-aiagentre-wire).
