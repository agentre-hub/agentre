...
// ── 会话通知出口:推出去的是预览帧,落库的是块 ─────────────────────────────────
//
// 通知日志退役之后(规格 2026-09-05 决策 1),出口只剩一件事:把这一帧推给此刻活着的
// 那条连接。内容的事实由块表持有 —— 推出去的这一帧是**预览**,即时呈现用,不带编号、
// 不参与对端的游标推进与去重(决策 4)。
...
// 随即指向一个宿主认不回来的位置 —— 那正是这一轮要消灭的问题 B。
...
func TestRuntime_Emit_EventFramesAreMarkedPreview(t *testing.T) {
	...
	for _, frame := range frames {
		...
		assert.True(t, ef.Preview, "实时事件帧是预览帧;同一段内容的事实在块表里")
	}
