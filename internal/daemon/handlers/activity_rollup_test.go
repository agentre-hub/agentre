...
func TestActivityRollup_CountsSessionsByDayAndDimensions(t *testing.T) {
	...
	sessions.EXPECT().ListCreatedSince(gomock.Any(), int64(0)).Return([]handlers.SessionRecord{
...
func TestActivityRollup_SinceDayNarrowsTheAnswer(t *testing.T) {
	ctx, sessions, h := setupActivityTest(t)
	// 下界进的是查询:窗口外那条(08-20)根本不该被读回来 —— 此前它是读回来之后
	// 在内存里被丢掉的。
	sessions.EXPECT().ListCreatedSince(gomock.Any(), atUTC(t, "2026-08-28 00:00")).Return([]handlers.SessionRecord{
