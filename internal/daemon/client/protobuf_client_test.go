...
func TestDialProtobuf_GivenBinaryDaemon_WhenCallingTypedMethod_ThenNegotiatesProtocolAndSignalsDisconnect(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		gotRequest <- request
		encodedResponse, err := proto.Marshal(&agentrewire.AuthAccountResponse{Ok: true, InstanceUuid: "uuid-1", ProtocolVersion: wireversion.Protocol, MinSupportedProtocolVersion: wireversion.MinSupported})
		if err != nil {
			return
		}
...
func TestDialRelayProtobuf_GivenAccountCredential_WhenAuthenticating_ThenUsesTypedPayload(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		got <- request
		encodedResponse, _ := proto.Marshal(&agentrewire.AuthAccountResponse{Ok: true, ProtocolVersion: wireversion.Protocol, MinSupportedProtocolVersion: wireversion.MinSupported})
