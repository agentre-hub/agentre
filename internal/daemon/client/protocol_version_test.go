...
// Given an agentred that leaves the protocol version field unset, When it
// answers the handshake, Then proto3 hands us the empty string and it must be
// read as a mismatch, never as "same version". This is the whole reason the
// check cannot be written as `peer != "" && peer != ours`.
