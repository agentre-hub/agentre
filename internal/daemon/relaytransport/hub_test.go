...
// TestHubLink_GivenServerURLProvider_WhenTheDaemonIsNotLoggedInYet_ThenItWaitsAndConnectsAfterLogin
// 覆盖「先起服务、后 agentred login」这条路径：daemon 启动时还没登录，链路必须留在
// 重试循环里等，而不是当作「不存在」——login 是另一个进程，它写完 state.json 就退出，
// 没有任何东西会回来把链路建起来。
func TestHubLink_GivenServerURLProvider_WhenTheDaemonIsNotLoggedInYet_ThenItWaitsAndConnectsAfterLogin(t *testing.T) {
	upgrader := websocket.Upgrader{}
	conns := make(chan *websocket.Conn, 4)
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		ws, err := upgrader.Upgrade(w, r, nil)
	...
	case <-conns:
	case <-time.After(2 * time.Second):
		t.Fatal("登录之后同一个 Run 循环应当连上去，不需要重启进程")
	}
}

// 未登录是稳态,不是反复发生的故障:LAN-only 的 daemon 可以一直不登录,而重试退避
...
// 重试永远不会成功——机器要么已被解除授权，要么它登录的那个账号在服务端已经不存在
// （例如库被重建）。这两种都只能靠人 logout 后重新 login，所以这一行必须把话说清楚，
...
// 重连后的正常帧照常送达。
//
// 直连那条 WebSocket 早就封了顶(protorpc/transport.go 的 SetReadLimit),中继这条
// 一直没有 —— 对端说多大就分配多大。中继帧来自账号服务器转发的另一台设备,不是
// 本机可信输入,所以两条路必须同样有界。
//
// 断开而不是「跳过这一帧」是 WebSocket 的既定语义(gorilla 超限即回 1009 关闭),
// 也正确:超限说明对面要么坏了要么不怀好意,继续按帧边界读下去没有意义。
func TestHubLink_GivenAnOversizedRelayFrame_WhenReading_ThenItIsRejectedAndTheLinkRecovers(t *testing.T) {
	const limit = 1 << 10
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		defer func() { _ = ws.Close() }()
		if attempts.Add(1) == 1 {
			// 超限帧:发完就等对面按 1009 关连接。
			_ = ws.WriteMessage(websocket.BinaryMessage, make([]byte, limit*4))
			_, _, _ = ws.ReadMessage()
			return
		}
		_ = ws.WriteMessage(websocket.BinaryMessage, []byte("small"))
		_, _, _ = ws.ReadMessage()
	}))
	defer server.Close()
	...
	select {
	case frame := <-link.Receive():
		// 超限帧一旦被投递,说明上限没生效 —— 它比 limit 大得多,一眼可辨。
		require.Equal(t, []byte("small"), frame.Payload,
			"超限帧不得投递;拿到的必须是重连之后那条正常帧")
	case <-time.After(3 * time.Second):
		t.Fatal("重连后的正常帧没有送达 —— 链路没能从超限帧里恢复")
	}
	assert.GreaterOrEqual(t, int(attempts.Load()), 2, "超限必须断连重拨")
}
