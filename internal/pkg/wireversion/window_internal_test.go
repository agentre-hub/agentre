...
func TestWindowMatch(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:         "peer that leaves its floor empty rejects even when its protocol is in range",
			selfProtocol: "0.3.0", selfMinSupported: "0.1.0",
			peerProtocol: "0.2.0", peerMinSupported: "",
			want: false,
		},
		{
			name:         "peer that leaves its protocol empty rejects",
			selfProtocol: "0.1.0", selfMinSupported: "0.1.0",
		...
		{
			name:         "malformed peer floor rejects",
			selfProtocol: "0.1.0", selfMinSupported: "0.1.0",
			peerProtocol: "0.1.0", peerMinSupported: "not-a-version",
			want: false,
		},
	}
