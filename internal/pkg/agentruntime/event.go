...
// UserMessageEvent (R18):一轮的用户消息,携带该轮的用户文本与提交方设备身份
// (指纹 + 显示名)。它由宿主的用户消息行投影而来,以**持久帧**的身份出去一次 ——
// 消费方据它在转录里落成一行用户消息 + 来源标识(「来自 <设备名>」),并据它区分
// 「别人发起的一轮」与「自主续轮」(纯 assistant,没有这一帧)。
//
// 只有一个来源:从前宿主还会在事件流开头另注入一条同样内容的**预览标记**,而拿帧
// 重建转录的消费方对每条 user_message 都新建一条用户消息、不去重 —— 同一句话于是
// 画出两条。本机(桌面端)自己发的那一句不带 SourceDevice,不盖来源,单端界面零变化。
