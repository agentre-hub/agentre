...
type SessionCursorPort interface {
	...
	// ok=false：此时记录的游标指向另一个 daemon 实例的帧编号，调用方不得据此增量拉取。
	LoadCursor(ctx context.Context, sessionID int64, daemonFingerprint devicefp.Carrier) (seq int64, ok bool, err error)
	// SaveCursor 记录 sessionID 已消费到 seq。daemonFingerprint 是签发这个 seq
	// 的 daemon 实例标识 —— 与读侧对称:seq 离开签发它的实例就是个错的数字,
	// 会话已改绑到别的 daemon 时这次写入落空(不报错),下次重连至多重复拉取,
	// 而不会从一个远超新实例帧编号的位置往后拉、把整段 transcript 丢掉。
