...
const (
	...
	EventToolApprovalResolved  EventKind = "tool_approval_resolved"
...
type SubagentInfo struct {
	...
	// 如 "Agent terminated early due to an API error: ...")。不填 Runs 的单运行
	// 生产者只有这一处承载结论。
...
// the user prompt of an auto-started next turn. A claudecode Stop hook with
// decision=block is not used for this: it surfaces as a red "Stop hook error"
// banner in Claude's TUI and drops messages that arrive during the hook's own
// execution.
//
