...
func buildGoldenFrames(t *testing.T) []goldenFrame {
	...
	// 一条还没跑过第一轮的会话:标题 / Agent 标识 / provider_session_id 如实留空
	// (omitempty 直接省略键,不填占位名)。
	minimalSummary := SessionSummary{
	...
	return []goldenFrame{
		...
		{name: "session-summary-minimal", body: minimalSummary},
		{name: "session-list-result", body: SessionListResult{
			Sessions: []SessionSummary{newSummary, minimalSummary},
		}},
