...
func TestEventNotificationPreservesEveryField(t *testing.T) {
	...
	specimens := []agentruntime.Event{
		...
		agentruntime.ToolApprovalRequested{ToolKey: "ctl", RequestID: "ta", ToolName: "ctl_delete_org", ToolInput: json.RawMessage(`{"command":"agrctl delete org x"}`)},
		agentruntime.ToolApprovalResolved{RequestID: "ta", Status: "approved", Result: "deleted"},
