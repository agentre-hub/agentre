...
func WireNotificationToProto(method string, params any) (*agentrewire.RpcNotification, error) {
	...
	case wire.NotifyTurnStarted:
		frame, ok := turnStartedFrame(params)
		if !ok {
			return nil, fmt.Errorf("protowire: %s 参数类型为 %T", method, params)
		}
		return &agentrewire.RpcNotification{Payload: &agentrewire.RpcNotification_TurnStarted{TurnStarted: &agentrewire.TurnStartedNotification{ConversationId: frame.ConversationID, Seq: frame.Seq}}}, nil
	default:
...
func ProtoNotificationToWire(notification *agentrewire.RpcNotification) (string, any, error) {
	...
	case *agentrewire.RpcNotification_TurnStarted:
		...
		return wire.NotifyTurnStarted, &wire.TurnStartedFrame{ConversationID: value.GetConversationId(), Seq: value.GetSeq()}, nil
	default:
...
func turnStartedFrame(value any) (wire.TurnStartedFrame, bool) {
	switch v := value.(type) {
	...
	return wire.TurnStartedFrame{}, false
}
