...
// 会话摘要的编解码只许有这一处。
//
// 桌面端出站应答、agentred 出站应答、桌面端解对端清单、桌面端解 agentred 清单都用它。
// 抄成几份的话,proto3 分辨不了「没发这一格」与「发了零值」—— 加一个字段时漏掉其中
// 任何一处,编译器不会报错,往返测试也不会红:发的是零值、解的也是零值,只是标题变空、
// 最后活动时间变 0、思考力度悄悄退回「跟随后端配置」。这类缺陷只会在用户的清单里露面。
// TS 侧同一件事也只有一份(agentre-wire 的 sessionListFromProtobuf)。
