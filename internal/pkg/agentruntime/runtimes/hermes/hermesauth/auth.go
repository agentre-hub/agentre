// Package hermesauth is the side-effect-free client of a gated `hermes serve`'s
// native auth endpoints (provider directory, PKCE password login, refresh and
// WebSocket tickets). It registers no runtime, so credential code that must not
// pull the Hermes runtime in (agentred, backendcred) can depend on it.
package hermesauth
...
// ErrProtocol marks a reachable server whose auth endpoints did not answer as
// Hermes does. Its text matches the runtime's gateway protocol error so the
// surfaced message is the same on both paths.
var ErrProtocol = errors.New("hermes gateway: unexpected protocol")

// httpTimeout bounds each auth request when the caller supplies no client.
...
// RandomToken returns bytesLen random bytes as unpadded base64url, used for the
...
func FetchWSTicket(ctx context.Context, baseURL, accessToken string, client *http.Client) (string, error) {
	...
	if strings.TrimSpace(env.Ticket) == "" {
		return "", fmt.Errorf("%w: ws-ticket response carried no ticket", ErrProtocol)
	}
...
func beginNativeAuthorize(ctx context.Context, client *http.Client, base, provider, challenge, state string) (*http.Cookie, error) {
	...
	if resp.StatusCode >= 400 {
		return nil, fmt.Errorf("%w: native authorize returned %s", ErrProtocol, resp.Status)
	}
	...
	return nil, fmt.Errorf("%w: native authorize did not set the PKCE cookie", ErrProtocol)
}
...
func passwordLoginCode(
	...
	default:
		return "", fmt.Errorf("%w: password-login returned %s", ErrProtocol, resp.Status)
	}
	...
	if err := json.Unmarshal(raw, &env); err != nil {
		return "", fmt.Errorf("%w: decode password-login response: %v", ErrProtocol, err)
	}
	if !env.OK {
		return "", fmt.Errorf("%w: password-login did not report ok", ErrProtocol)
	}
	next, err := url.Parse(strings.TrimSpace(env.Next))
	if err != nil || next.Host == "" {
		return "", fmt.Errorf("%w: password-login returned an invalid next URL", ErrProtocol)
	}
	if got := next.Query().Get("state"); got != state {
		// Never echo the server's forged state or the credentials.
		return "", fmt.Errorf("%w: password-login state mismatch", ErrProtocol)
	}
	code := next.Query().Get("code")
	if code == "" {
		return "", fmt.Errorf("%w: password-login returned no code", ErrProtocol)
	}
	return code, nil
}
...
func authRequest(ctx context.Context, client *http.Client, method, rawURL string, headers map[string]string, body, out any) error {
	...
	if resp.StatusCode/100 != 2 {
		return fmt.Errorf("%w: %s returned %s", ErrProtocol, rawURL, resp.Status)
	}
	if out == nil {
		return nil
	}
	if err := json.Unmarshal(raw, out); err != nil {
		return fmt.Errorf("%w: decode %s: %v", ErrProtocol, rawURL, err)
	}
	return nil
}
