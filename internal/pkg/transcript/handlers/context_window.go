...
//
// 落库故意收在这个接口里(而不是整行回写 session):后者会把调用方手上那份实体的
// 每一列都写回去。**带外轮**(自主续轮 / 后台 subagent 活动轮)
...
func (h ContextWindowUpdatedHandler) Apply(ctx context.Context, ev agentruntime.Event, _ *turn.Accumulator, emit turn.Emitter, tc *turn.TurnContext) error {
