...
//
// 它此前叫 internal/daemon/protobufadapter,却已经被 internal/peer 反向 import ——
// 位置本身就是「这段代码没有主人」的信号:两种执行端共用的注册面,挂在其中一种的
// 名下。而它当时持有 daemon 四个包的具体类型,单纯 git mv 会让横切叶子层反向依赖
// 宿主,正好把要拆的耦合固化下来。
//
// 换成接口之后依赖方向只剩一条:宿主 → 本包。两种执行端各自交出自己的实现,谁也
...
// 种说法,调用方据此换一台机器。这条纪律必须对每一族都成立:从前 transcriptimport
// 那一族漏了 nil 守卫,桌面端因此挂上了四个持有 nil 的 handler,进去就解空指针,
// 对端收到的是 -32603 internal —— 一个只会把人引去查目标机器日志的答复。
//
