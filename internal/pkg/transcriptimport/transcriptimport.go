// Package transcriptimport 定义会话的后端无关导入契约:发现、打开和回放。
// Source 只读磁盘档案;Turns 以流式回调逐轮输出,不要求整份转录载入内存。
...
// Locator 是由 Source 解释的不透明定位符,消费方只负责原样回传。
type Locator string
...
// Matches 判断候选是否满足筛选条件。Limit 由合并排序后的调用方处理。
