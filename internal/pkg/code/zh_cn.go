...
var zhCN = map[int]string{
	...
	LLMProviderNoEnabledDefault:     "启用前必须先为该供应商设置一个启用中的默认模型",
	LLMProviderReferenced:           "该供应商正被 Backend / 会话 / 路由引用，删除需要二次确认",
	LLMProviderModelNotFound:        "模型不存在",
	LLMProviderModelNotOwned:        "该模型不属于此供应商",
	LLMProviderModelDisabled:        "该模型已停用",
	LLMProviderModelIsDefault:       "默认模型不能停用或删除，请先指定其它默认模型",
	LLMProviderModelReferenced:      "该模型正被 Backend / 会话 / 路由引用，删除需要二次确认",
	...
	HermesLoginRequired:       "该 Hermes 服务需要登录，请先输入账号密码登录",
	HermesLoginExpired:        "Hermes 登录已过期，请重新登录",
	HermesLoginRejected:       "用户名或密码错误",
	HermesRateLimited:         "登录尝试过于频繁，请稍后再试",
	HermesProviderUnsupported: "该认证方式需要浏览器授权，当前版本暂不支持",
	HermesProviderUnavailable: "身份提供方暂时不可用，请稍后再试",
	...
	ChatBackendRemoteHermesUnavailable: "该 Hermes 后端绑定到了远端设备，但 Hermes 无法派发到 agentred。请把它指向本机运行的 `hermes serve`。",
	...
	ChatExecTargetOverrideUnavailable:  "这一档执行目标现在不可用，无法指定",
	ChatExecTargetOverrideNotInList:    "指定的执行目标不在这个 Agent 的执行目标列表里",

	ChatExecTargetLineFormat:             "%d. backend #%d（%s）：%s",
	ChatExecTargetDeviceLocal:            "本机",
	ChatExecTargetDeviceRemote:           "设备 %s",
	...
	ChatBackendHintProviderTypeMismatch:  "请先在设置 → LLM 供应商选择与该 Agent 后端匹配的类型",
	ChatBackendHintRemoteProviderMissing: "远端 agentred 未配置该供应商，请前往「远端设备」页在对应设备上添加并填写 API Key",
	ChatBackendHintGatewayNotRunning:     "本地网关未启动，CLI 后端暂不可用",
	ChatBackendHintRemoteOpenClaw:        "远端 OpenClaw 暂不可用：agentred 尚无安全的 secret enrollment/reference",
	ChatBackendHintUnknownType:           "未知 Agent 后端类型",
	ChatAgentNoBackendHint:               "该 Agent 还没配置后端，请在组织架构页选择一个 Agent 后端",
	ChatSystemAgentNoBackendHint:         "CEO 助手还没配置后端，请在组织架构页选择一个 Agent 后端",
	ChatBackendHintRemoteHermes:          "远端 Hermes 暂不可用：Hermes 后端运行在 Agentre 桌面端，不能在 agentred 上运行",
	...
	RemoteDeviceProtocolUnsupported:     "这台机器上的 agentred 不认识桌面端的通信协议。请把它升级到与桌面端相同的版本（在那台机器上重新部署 agentred）后重试。",
	RemoteDeviceProtocolVersionMismatch: "这台机器上的 agentred 与桌面端的通信协议版本不一致。请把它升级到与桌面端相同的版本（在那台机器上重新部署 agentred）后重试。",
	...
	RemoteDeviceUpgradeUnavailable:      "当前是开发构建，没有对应的发布可升级，不提供远端 agentred 一键升级",
	...
	ChatImportBackendMismatch:        "选中的 Agent 后端与这条会话的后端不一致,换一个同后端的 Agent",
	ChatImportTranscriptOpenFailed:   "这条会话的转录打不开:文件可能已被删除或损坏",
	ChatImportTranscriptReplayFailed: "读取这条会话的转录时失败",
	ChatImportTranscriptEmpty:        "这条转录解不出任何一轮对话",
	ChatImportAgentNoBackend:         "这个 Agent 还没配置后端,先去组织架构里选一个",
	ChatImportGapThinking:            "思维过程在磁盘上是加密内容,导入后思维块为空;正文、工具调用与结果完整。",
	ChatImportGapSubagentInternals:   "子代理的内部过程没有留在磁盘上,导入后只有它的结果。",
	ChatImportGapTruncated:           "这条会话的部分内容在磁盘上已被截断,导入的是剩下的部分。",
	...
	PortForwardInvalidTarget: "目标写法不对:接受 3000 / host:port / http(s)://host[:port] 三种写法",
}
