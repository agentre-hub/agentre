...
// PeerListSessions 列出一台远端桌面端上的一页会话（R19 设备页下钻）。
//
// 按页要而不是整份：那台机器上可能有几千条对话，整份搬回来既撑 Wails 桥，
// 也让展开那一下等在解码上。
func (a *App) PeerListSessions(req peer_svc.ListSessionsRequest) (*wire.SessionListResult, error) {
	if svc := peerSvcAccessor(); svc != nil {
		return svc.ListSessions(a.ctx, req)
	}
