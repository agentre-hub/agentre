...
type Agent struct {
	...
	// AgentBackendID is derived from the first execution target; zero means none.
	AgentBackendID int64  `gorm:"-"`
	...
	// SkillsJSON is the raw skills payload passed to the repository; it is not persisted.
	SkillsJSON string `gorm:"-"`
