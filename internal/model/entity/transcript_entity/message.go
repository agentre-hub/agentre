// Package transcript_entity 维护转录（消息 + 块）的充血实体：块拆分、CheckpointBlocks
// 差分、正文编解码。两个宿主（桌面端 / agentred）各自一个 SQLite 库，共用同一份实体与
// 仓储代码（决策 8）；会话行不在这里，仍归各宿主自己的域（桌面端见 chat_entity）。
