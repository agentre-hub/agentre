// Package transcript_repo 提供转录（消息 + 块）的持久化访问：写入、块拆分、
// CheckpointBlocks 差分、按定位键与类型的点查、按筛选条件收窄的删除。两个宿主
// （桌面端 / agentred）各自一个 SQLite 库，按 db.Ctx(ctx) 写，共用同一份实现（决策 8）；
// chat_repo 只留会话。
...
//go:generate mockgen -source message.go -destination mock_transcript_repo/mock_message.go
...
type MessageRepo interface {
	...
	FillBlocksByType(ctx context.Context, msgs []*transcript_entity.Message, types []string) error
	...
	CheckpointBlocks(ctx context.Context, m *transcript_entity.Message, prevBlocksJSON string) error
	...
	FindAssistantBySubagentToolCallID(ctx context.Context, sessionID int64, toolCallID string) (*transcript_entity.Message, error)
	// LatestAssistant 取某会话 seq 最大的一条 assistant 消息(无 → nil,nil)。
	// 用于 peek 运行中子任务的当前输出（read 工具的 running 分支）。
	LatestAssistant(ctx context.Context, sessionID int64) (*transcript_entity.Message, error)
...
func (r *messageRepo) List(ctx context.Context, sessionID int64) ([]*transcript_entity.Message, error) {
...
func (r *messageRepo) ListMeta(ctx context.Context, sessionID int64) ([]*transcript_entity.Message, error) {
...
func (r *messageRepo) FillBlocksByType(ctx context.Context, msgs []*transcript_entity.Message, types []string) error {
	if len(types) == 0 {
...
func (r *messageRepo) CheckpointBlocks(ctx context.Context, m *transcript_entity.Message, prevBlocksJSON string) error {
...
func (r *messageRepo) Update(ctx context.Context, m *transcript_entity.Message) error {
	m.Updatetime = time.Now().UnixMilli()
	return db.Ctx(ctx).Transaction(func(tx *gorm.DB) error {
		txCtx := db.WithContextDB(ctx, tx)
		if err := tx.Save(m).Error; err != nil {
...
func (r *messageRepo) UpdateUsage(ctx context.Context, id int64, u MessageUsage) error {
	return db.Ctx(ctx).Model(&transcript_entity.Message{}).
		Where("id = ?", id).
...
func (r *messageRepo) Find(ctx context.Context, id int64) (*transcript_entity.Message, error) {
...
func (r *messageRepo) LatestAssistant(ctx context.Context, sessionID int64) (*transcript_entity.Message, error) {
	var m transcript_entity.Message
	err := db.Ctx(ctx).
