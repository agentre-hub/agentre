...
// negative namespace holding the hidden original rows. The mapping lands on odd
// negatives only, so a recovery namespace can never be mistaken for -sessionID.
