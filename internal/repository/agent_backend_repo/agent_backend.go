...
func (r *agentBackendRepo) Create(ctx context.Context, b *agent_backend_entity.AgentBackend) error {
	...
	b.EnsureSyncID()
	if err := b.MarshalConfig(); err != nil {
		return err
	}
...
func (r *agentBackendRepo) Find(ctx context.Context, id int64) (*agent_backend_entity.AgentBackend, error) {
	...
	return out, hydrateConfig(out)
}

func (r *agentBackendRepo) BatchFind(ctx context.Context, ids []int64) (map[int64]*agent_backend_entity.AgentBackend, error) {
	out, err := repoquery.ActiveMap[agent_backend_entity.AgentBackend](ctx, "id", ids, func(b *agent_backend_entity.AgentBackend) int64 {
		return b.ID
	})
	if err != nil {
		return nil, err
	...
	for _, b := range out {
		if err := hydrateConfig(b); err != nil {
			return nil, err
		}
	}
	return out, nil
...
// agent_backend_entity/config.go）。**每一条读路径都要经过它** —— 漏掉一条不会
// 报错，只会让那条路径交出一个 Sandbox / 网关地址是空串的后端，而空串在这几格上
// 都是合法取值（走 CLI 默认 / 还没配），于是子进程被静默地以错误配置起来。
//
// 与写口的 MarshalConfig 是同一条纪律的两半：入库前收进一列，出库后摊回字段。
func hydrateConfig(rows ...*agent_backend_entity.AgentBackend) error {
	for _, b := range rows {
		if b == nil {
			continue
		}
		if err := b.UnmarshalConfig(); err != nil {
			return err
		}
	}
	return nil
