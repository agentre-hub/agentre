...
// TestClaimUnowned_GivenTheBoardKinds_ResolvesEveryTable 看板并入同步组带来的三个
// 对象类型必须都在表名白名单里：认领（R12a）逐个跑一遍同步组的全部类型，任何一个
// 不在白名单里就在那一次调用上报 ErrUnknownKind，把整轮同步——连同已经在用的九个
// 类型——一起打断。
func TestClaimUnowned_GivenTheBoardKinds_ResolvesEveryTable(t *testing.T) {
	for _, tc := range []struct{ kind, table string }{
		{syncwire.KindLabel, "labels"},
		{syncwire.KindIssue, "issues"},
	} {
		ctx, _, mock := testutils.Database(t)
		mock.ExpectQuery("SELECT rowid,sync_id,sync_version FROM `" + tc.table +
			"` WHERE \\(sync_account_id = 0 AND sync_deleted_at = 0\\) AND status = \\?").
			WithArgs(consts.ACTIVE).
			WillReturnRows(sqlmock.NewRows([]string{"rowid", "sync_id", "sync_version"}))

		rows, err := NewSyncState().ClaimUnowned(ctx, tc.kind, 7)
...
// TestClaimUnowned_GivenIssueLabels_SkipsStatusFilter 关联表是硬删，没有 status 列
// ——把它拼进 SQL 会让认领在这张表上直接报错。
func TestClaimUnowned_GivenIssueLabels_SkipsStatusFilter(t *testing.T) {
	ctx, _, mock := testutils.Database(t)
	mock.ExpectQuery("SELECT rowid,sync_id,sync_version FROM `issue_labels` WHERE sync_account_id = 0 AND sync_deleted_at = 0").
		WillReturnRows(sqlmock.NewRows([]string{"rowid", "sync_id", "sync_version"}))

	rows, err := NewSyncState().ClaimUnowned(ctx, syncwire.KindIssueLabel, 7)
	require.NoError(t, err)
	assert.Empty(t, rows)
	assert.NoError(t, mock.ExpectationsWereMet())
}
