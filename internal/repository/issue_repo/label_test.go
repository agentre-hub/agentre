...
// TestIssueLabelListRowsByLabel 与 ListRowsByIssue 对称的整行 lister：删一个标签时，
// 被摘掉的每一条关联行要凭那一行**自己的**同步标识落墓碑（关联表是硬删，R6），
// 而只回 issue_id 的取数拿不出标识。
func TestIssueLabelListRowsByLabel(t *testing.T) {
	ctx, _, mock := testutils.Database(t)
	repo := issue_repo.NewIssueLabel()
	mock.ExpectQuery("SELECT \\* FROM `issue_labels` WHERE label_id = \\? ORDER BY issue_id ASC").
		WithArgs(int64(3)).
		WillReturnRows(sqlmock.NewRows([]string{"issue_id", "label_id", "sync_id", "sync_version"}).
			AddRow(int64(11), int64(3), "link-a", int64(4)).
			AddRow(int64(12), int64(3), "link-b", int64(5)))

	rows, err := repo.ListRowsByLabel(ctx, 3)
	require.NoError(t, err)
	require.Len(t, rows, 2)
	assert.Equal(t, int64(11), rows[0].IssueID)
	assert.Equal(t, "link-a", rows[0].SyncID)
	assert.Equal(t, int64(5), rows[1].SyncVersion)
	assert.NoError(t, mock.ExpectationsWereMet())
}
