...
type IssueLabelRepo interface {
	...
	// ListRowsByLabel 返回一个标签当前挂在的全部关联行（整行，带同步元数据）。
	// 与 ListRowsByIssue 对称、同一条理由：删一个标签时被摘掉的每一条关联行要凭
	// 那一行**自己的**同步标识落墓碑（R6），DeleteByLabel 只回一个 error，拿不出
	// 标识——所以快照必须在它之前取。
	ListRowsByLabel(ctx context.Context, labelID int64) ([]*issue_entity.IssueLabel, error)
...
func (r *issueLabelRepo) ListRowsByLabel(ctx context.Context, labelID int64) ([]*issue_entity.IssueLabel, error) {
	var rows []*issue_entity.IssueLabel
	err := db.Ctx(ctx).Where("label_id = ?", labelID).
		Order("issue_id ASC").Find(&rows).Error
	return rows, err
}
