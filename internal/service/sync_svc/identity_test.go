...
// 之后本机就把 B 的 1 号用户认成 A 的 1 号用户。
//
// 修法是让这个整数变成**本机**为 (server 地址, 远端用户主键) 分配的代理键
// （sync_account_repo）：同一对永远同一个键，不同对永远不同。
//
// 规格 2026-09-04 决策 1 之后，上一个账号的行**会**被认领进新账号并上行——所以这里
// 守的不再是「一条都不推」，而是代理键本身认对了没有：认对了，那一行是以一次
// **认领**（新账号、基版本 0）过去的；认错了，它会被当成「还是同一个账号」而带着
// 上一套序列的基版本原样续推，那正是这个代理键要防的静默错账。
func TestSyncOnce_GivenAnotherServerWithTheSameRemoteUserID_ClaimsThemAsANewAccount(t *testing.T) {
	h := newHarness(t, true)
	ctx := context.Background()
	h.adapter.rows["p-1"] = "Alpha"
	// 一行属于 A 上那个账号的项目。
	...
	assert.Zero(t, h.transport.pushed[0][0].BaseVersion,
		"以一次认领过去：带上 A 序列的基版本 500 就说明两套 server 的 7 号被认成了同一个账号")
	assert.NotEqual(t, int64(7), h.state.meta["project:p-1"].SyncAccountID,
		"B 上的 7 号用户必须拿到另一个本机代理键")
}
