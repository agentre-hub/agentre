...
// TestPull_GivenAnIssueLabelPointingAtNothing_StillCompletesTheRound 孤儿关联行到达
// 时不许卡住整轮下行。
//
// 规格 2026-09-04 之后，本端发起的删除**会**把关联行的墓碑一起下发（任务走
// issueAdapter.children，标签走 issue_svc.DeleteLabel 的删前快照），所以孤儿不再从
// 这里产生。但它仍然会到达：旧版本客户端推上来的、以及那之前就留在账号里的存量。
// 落不了地时必须被**跳过**——一条落不了地的行如果让整轮下行报错，一个孤儿就能把此后
// 每一轮、每一种对象类型全部卡死，正是表名白名单那一类的故障形状。
//
...
// TestIssueAdapter_ChildrenReturnsItsLinkRows 规格 2026-09-04：删一个任务时，它挂着的
// 每一条关联行一并落墓碑（R6）。此前三个看板适配器都用 baseAdapter 的空 children，
// 于是 server 与别的机器上留下一批指向已删任务的活关联行——新设备全量拉取时它们的
// 两个引用都解析不出，按 R2a 暂缓 30 天后以「引用丢失」进「没能同步的改动」。
//
// 墓碑凭**关联行自己的**同步标识上行：关联表是联合主键，本地 ID 在另一台机器上指向
// 完全不同的两行。
func TestIssueAdapter_ChildrenReturnsItsLinkRows(t *testing.T) {
	ctrl := gomock.NewController(t)
	state := mock_syncstate_repo.NewMockSyncStateRepo(ctrl)
	state.EXPECT().FindLocalID(gomock.Any(), syncwire.KindIssue, "issue-1").Return(int64(9), nil)
	syncstate_repo.RegisterSyncState(state)
	links := mock_issue_repo.NewMockIssueLabelRepo(ctrl)
	links.EXPECT().ListRowsByIssue(gomock.Any(), int64(9)).Return([]*issue_entity.IssueLabel{
		{IssueID: 9, LabelID: 1, SyncMeta: syncmeta_entity.SyncMeta{SyncID: "link-1", SyncVersion: 4}},
		{IssueID: 9, LabelID: 2, SyncMeta: syncmeta_entity.SyncMeta{SyncID: "link-2", SyncVersion: 5}},
	}, nil)
	issue_repo.RegisterIssueLabel(links)

	kids, err := issueAdapter{}.children(context.Background(), "issue-1")
	require.NoError(t, err)
	require.Len(t, kids, 2)
	assert.Equal(t, syncwire.KindIssueLabel, kids[0].Kind)
	assert.Equal(t, "link-1", kids[0].SyncID)
	assert.Equal(t, int64(4), kids[0].Version)
	assert.Zero(t, kids[0].LocalID, "关联表是联合主键，没有本地自增 ID 可交")
	assert.Equal(t, "link-2", kids[1].SyncID)
}

// TestIssueAdapter_ChildrenGivenUnknownSyncID_ReturnsNothing 本机没有这一行时不该
// 编出任何子行墓碑。
func TestIssueAdapter_ChildrenGivenUnknownSyncID_ReturnsNothing(t *testing.T) {
	ctrl := gomock.NewController(t)
	state := mock_syncstate_repo.NewMockSyncStateRepo(ctrl)
	state.EXPECT().FindLocalID(gomock.Any(), syncwire.KindIssue, "gone").Return(int64(0), nil)
	syncstate_repo.RegisterSyncState(state)

	kids, err := issueAdapter{}.children(context.Background(), "gone")
	require.NoError(t, err)
	assert.Empty(t, kids)
}

// TestIssueAdapter_ChildrenSkipsLinksAlreadyTombstoned 已经落过墓碑的关联行不再落
// 第二条。
//
// 那一列非 0 的意思就是「这条删除已经送达过 server」（applyPushResult 与
// saveInboundMeta 写它，ListUnsyncedTombstones 靠它筛掉送达过的），全仓每一处编墓碑
// 的地方都拿它当判据。这里少了这道判据会出一条会**堆积**的错账：任务的墓碑上行被
// 单条拒（载荷过不了守卫、或对象类型不认），本机那行就一直是 status = DELETE 而
// sync_deleted_at 仍为 0，reconcileLostTombstones 于是每一轮重新入队它 → 每一轮
// children 重发一遍关联行的墓碑 → server 说「已是墓碑」→ acceptRemoteTombstone
// 每一轮往「没能同步的改动」里记一条。用户看着那张列表无休止地长出自己从没动过的
// 关联行。
func TestIssueAdapter_ChildrenSkipsLinksAlreadyTombstoned(t *testing.T) {
	ctrl := gomock.NewController(t)
	state := mock_syncstate_repo.NewMockSyncStateRepo(ctrl)
	state.EXPECT().FindLocalID(gomock.Any(), syncwire.KindIssue, "issue-1").Return(int64(9), nil)
	syncstate_repo.RegisterSyncState(state)
	links := mock_issue_repo.NewMockIssueLabelRepo(ctrl)
	links.EXPECT().ListRowsByIssue(gomock.Any(), int64(9)).Return([]*issue_entity.IssueLabel{
		{IssueID: 9, LabelID: 1, SyncMeta: syncmeta_entity.SyncMeta{
			SyncID: "link-buried", SyncVersion: 4, SyncDeletedAt: 1700,
		}},
		{IssueID: 9, LabelID: 2, SyncMeta: syncmeta_entity.SyncMeta{SyncID: "link-live", SyncVersion: 5}},
	}, nil)
	issue_repo.RegisterIssueLabel(links)
	...
	assert.Equal(t, "link-live", kids[0].SyncID,
