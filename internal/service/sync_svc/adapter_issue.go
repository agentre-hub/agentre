...
// children 删一个任务时它挂着的关联行一并落墓碑（R6）。
//
// 没有这一步，server 与别的机器上会留下一批指向已删任务的活关联行：本机这一侧
// issue_repo.Issue().Delete 只把 issues.status 置 DELETE、不摘关联，对端的 remove
// 同样不摘，而 server 收到一条任务墓碑也不会去清关联行（它的级联只长在 web 的写
// 通道上）。那些孤儿行在新设备全量拉取时两个引用都解析不出，按 R2a 暂缓 30 天后
// 以「引用丢失」进「没能同步的改动」——用户看到一批自己从没动过的失败记录。
//
// 墓碑凭**关联行自己的**同步标识上行，LocalID 交 0：关联表是联合主键，本地 ID 在
// 另一台机器上指向完全不同的两行（与 project_agent 的成员关系同形）。
//
// 标签那一侧不在这里：issue_svc.DeleteLabel 先硬删关联行再交出标签的删除，等这里
// 去取时那些行已经不在库里了，因此快照由那条路径在硬删之前自己取（见它的注释）。
func (issueAdapter) children(ctx context.Context, syncID string) ([]relatedRow, error) {
	id, err := syncstate_repo.SyncState().FindLocalID(ctx, syncwire.KindIssue, syncID)
	if err != nil || id == 0 {
		return nil, err
	}
	rows, err := issue_repo.IssueLabel().ListRowsByIssue(ctx, id)
	if err != nil {
		return nil, err
	}
	out := make([]relatedRow, 0, len(rows))
	for _, row := range rows {
		// 已经落过墓碑的行不再落第二条：那一列非 0 的意思就是「这条删除已经送达过
		// server」（applyPushResult / saveInboundMeta 写它，ListUnsyncedTombstones 靠它
		// 筛掉送达过的）。任务软删不摘关联行，那些行会带着这个标记继续留在
		// issue_labels 里被 ListRowsByIssue 取到；而任务自己的墓碑若被单条拒，
		// reconcileLostTombstones 每一轮都会重新入队它、每一轮走到这里——不看这一列
		// 就每一轮往「没能同步的改动」里堆一条用户从没动过的关联行。
		if row.SyncID == "" || row.SyncDeletedAt != 0 {
			continue
		}
		out = append(out, relatedRow{
			Kind: syncwire.KindIssueLabel, SyncID: row.SyncID, Version: row.SyncVersion,
		})
	}
	return out, nil
}
