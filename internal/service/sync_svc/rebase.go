...
func (s *service) rebase(ctx context.Context, accountID int64) error {
	...
	if err := s.forgetServerVersions(ctx); err != nil {
		return err
	}
...
// forgetServerVersions 清掉本机**存活**行的同步版本号，不分账号。
//
...
// 按当前账号过滤等于一行都清不到；而跨账号唯一会撞上的那个固定同步标识
// （agent_entity.DefaultAgentSyncID）恰恰因此让新账号那份被守卫挡掉，随后被上一个
// 账号那份覆盖上去（守卫见 applyInbound，它读的 FindVersion 不带账号条件）。
//
// 软删行的版本号仓储那一侧刻意不清：那是「server 见过这一行」这条事实本身，
// ListUnsyncedTombstones 靠它找出没送达的删除（R6 的本机兜底）。
func (s *service) forgetServerVersions(ctx context.Context) error {
	for _, kind := range syncKinds {
		if err := syncstate_repo.SyncState().ResetVersions(ctx, kind); err != nil {
			return err
		}
	}
	return nil
