...
func registerPeerWaiterRepos(t *testing.T, ctrl *gomock.Controller, backend *agent_backend_entity.AgentBackend) *mock_chat_repo.MockSessionRepo {
	t.Helper()
...
func remoteWaiterBackend() *agent_backend_entity.AgentBackend {
	return &agent_backend_entity.AgentBackend{
		ID: 12, Type: string(agent_backend_entity.TypeClaudeCode), DeviceFingerprint: testDeviceFingerprint(7), Status: consts.ACTIVE,
	}
}
...
func TestPendingPeerSessionWaiters_GivenRemoteSessionBlockedOnDecision_ThenServesRemoteSnapshot(t *testing.T) {
	...
	sessRepo := registerPeerWaiterRepos(t, ctrl, backend)
	installPairedDevice(t, ctrl, 7)
	// 这一次落库写是**那一轮**借连接时记的执行归属（R15b），不是下面那次只读查询干
	// 的 —— 后者一次都不许写，由 registerPeerWaiterRepos 的严格替身钉死。
	sessRepo.EXPECT().UpdateExecDaemon(gomock.Any(), int64(42), int64(7), testDeviceFingerprint(7), int64(12)).
		Return(nil).Times(1)
...
func TestPendingPeerSessionWaiters_GivenNoLiveRemoteRuntime_ThenEmptyWithoutDialing(t *testing.T) {
	...
	_ = registerPeerWaiterRepos(t, ctrl, remoteWaiterBackend())
