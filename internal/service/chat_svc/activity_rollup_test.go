...
func TestActivityRollup_CountsDesktopSessionsByDayAndDimensions(t *testing.T) {
	...
	deps.session.EXPECT().ListForRollup(ctx, int64(0)).Return([]*chat_entity.Session{
...
// TestActivityRollup_PushesTheSinceDayDownToTheQuery 覆盖窗口下推:界面问的通常只是
// 最近 30 天,而此前是把库里全部会话读进内存再按天丢掉不要的 —— 为一张 30 天的图读
// 三年的会话。下界按调用方给的时区切,和分桶同一套日界。
func TestActivityRollup_PushesTheSinceDayDownToTheQuery(t *testing.T) {
	deps := setupPeerSessionTest(t)
	ctx := context.Background()
	deps.agent.EXPECT().List(ctx).Return([]*agent_entity.Agent{
	...
	// 2026-08-28 00:00 上海 = 2026-08-27 16:00 UTC = 1787846400000 毫秒。
	deps.session.EXPECT().ListForRollup(ctx, int64(1787846400000)).Return([]*chat_entity.Session{
		{ID: 41, AgentID: 7, Createtime: 1787846400000, LastMessageAt: 1787846400000, Status: consts.ACTIVE},
	}, nil)

	got, err := deps.svc.ActivityRollup(ctx, "2026-08-28", "Asia/Shanghai")
	require.NoError(t, err)
	require.Len(t, got, 1)
