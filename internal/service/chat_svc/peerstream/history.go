...
func (s *Publisher) peerPublication(sessionID int64, conversationID string) *peerSessionPublication {
...
func (s *Publisher) peerFlushLoop(publication *peerSessionPublication) {
...
func (s *Publisher) flushPeerPending(publication *peerSessionPublication) {
	publication.mu.Lock()
...
// (与从前同一判据)。
func (s *Publisher) deliverPeerPending(publication *peerSessionPublication, key string) {
	for {
		publication.mu.Lock()
...
func (s *Publisher) PullPeerSession(ctx context.Context, params wire.SessionPullParams, subscriber PeerSessionSubscriber) (wire.SessionPullResult, error) {
	if subscriber == nil {
...
// 持久帧**由 PublishMessageFrames 在它落库之后另行发出,并带着落库的编号。
//
// 从前这里给每一帧都发一个号:一次重启之后同一份内容会被重新编号,对端的游标随即
// 指向一个宿主认不回来的号(问题 B)。
func (s *Publisher) PublishEvent(sessionID int64, event agentruntime.Event) {
...
// PublishMessageFrames 把一条**已落库**的消息此刻投影出的持久帧发给对端。
//
...
// 发起方据此不推进游标,行为退回本轮之前。
func (s *Publisher) PublishMessageFrames(
...
// PublishTurnDone 在一轮收口时把这条 assistant 消息的持久帧整份发给对端订阅者。
//
...
func (s *Publisher) PublishTurnDone(ctx context.Context, sessionID int64, msg *chat_entity.Message) {
	_, _ = s.PublishMessageFrames(ctx, sessionID, msg, true)
}
