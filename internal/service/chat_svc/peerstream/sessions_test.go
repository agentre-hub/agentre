...
func TestAttachPeerSession_GivenLiveDesktopSession_ThenAddsAndCleansUpRemoteSubscriber(t *testing.T) {
	...
	assert.Equal(t, 1, deps.svc.SubscriberCount(41), "remote attaches alongside the desktop UI; it must not replace a local subscriber")

	close(subscriber.done)
	require.Eventually(t, func() bool {
		return deps.svc.SubscriberCount(41) == 0
