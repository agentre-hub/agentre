...
func TestAttachPeerSession_GivenHostRestartAfterLiveOutput_ThenPeerCursorStaysInsideTheSeqUniverse(t *testing.T) {
	...
	deps.svc.PublishEvent(41, agentruntime.TextDelta{Text: "live-"})
...
// 需要观察编号宇宙的用例走这条路。
func publishPeerDurableFrame(t *testing.T, svc *Publisher, sessionID, messageID int64, text string) {
	t.Helper()
	svc.PublishMessageFrames(context.Background(), sessionID, &chat_entity.Message{
...
// 确实是按 seq 递增重放的。
func peerSeqByContent(t *testing.T, svc *Publisher, ctx context.Context) map[string]int64 {
	t.Helper()
	subscriber := newRecordingPeerSubscriber()
...
func TestPublishPeerMessageFrames_GivenBlockPatchedInPlace_ThenOnlyTheChangedFrameTakesAFreshTailSeq(t *testing.T) {
	...
	deps.svc.PublishMessageFrames(ctx, 41, msg, true)
...
func TestAttachPeerSession_GivenFramesPublishedLiveThenHostRestart_ThenThePeerCursorSurvives(t *testing.T) {
	...
	deps.svc.PublishMessageFrames(ctx, 41, live, true)
	require.Eventually(t, func() bool {
		return len(subscriber.notifications()) == 2
	}, time.Second, time.Millisecond)
...
func TestAttachPeerSession_GivenTurnStillRunning_ThenTheGrowingTailWaitsForSettlement(t *testing.T) {
	...
	deps.svc.PublishMessageFrames(ctx, 41, assistant, true)
...
func TestPublishPeerMessageFrames_GivenFramesAllocated_ThenReturnsHighestSeq(t *testing.T) {
	...
	lowest, highest := deps.svc.PublishMessageFrames(ctx, 41, user, true)
	...
	lowest, highest = deps.svc.PublishMessageFrames(ctx, 41, withImage, true)
