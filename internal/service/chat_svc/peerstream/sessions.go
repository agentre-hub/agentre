...
func (s *Publisher) ListPeerSessions(ctx context.Context, params wire.SessionListParams) (*wire.SessionListResult, error) {
...
func (s *Publisher) CountPeerSessions(ctx context.Context) (*wire.SessionCountsResult, error) {
...
func (s *Publisher) AttachPeerSession(ctx context.Context, params wire.SessionAttachParams, subscriber PeerSessionSubscriber) (wire.SessionAttachResult, error) {
	if subscriber == nil {
...
func peerSessionSummary(
	...
	fingerprint devicefp.Initiator, projectSyncIDs map[int64]string,
...
func (s *Publisher) SubscriberCount(sessionID int64) int {
...
func desktopPeerFingerprint() (devicefp.Initiator, error) {
	...
	// 本机的承载者身份在这里换角色:交出去的这条会话由本机**发起**,对端拿到的
	// 是发起方指纹 —— 它可能根本不在对端的配对表里,所以不能当承载者用。
	return devicefp.Initiator(fingerprint), nil
}
