...
func TestPublishPeerEvent_GivenStalledSubscriber_ThenLocalPublishDoesNotBlockAndFrameStillDelivers(t *testing.T) {
	...
	case <-time.After(time.Second):
		t.Fatal("PublishEvent blocked on a stalled peer subscriber; a local turn would stall")
	}
...
// internal/pkg/transcript 的 TestProjectMessages_GivenTurnStats_ThenDoneCarriesThem。
func TestPublishPeerTurnDone_GivenFinishedTurn_ThenPeersSeeTurnStats(t *testing.T) {
	...
	deps.svc.PublishTurnDone(ctx, 41, &chat_entity.Message{
