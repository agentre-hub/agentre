...
// 判定规则与取值都归执行目标域所有（exec_target_svc.BlockReasonForBackend 是唯一判定点）；
// 这里别名过来，是因为 ChatAgentItem 把它当自己的 DTO 字段透给前端。
type BlockReason = exec_target_svc.BlockReason

const (
	// BlockReasonNoBackend 该 Agent 没绑后端（含 CEO）。
	BlockReasonNoBackend = exec_target_svc.BlockReasonNoBackend
	// BlockReasonBackendRequiresProvider 后端需要但找不到绑定的 LLM 供应商。
	BlockReasonBackendRequiresProvider = exec_target_svc.BlockReasonBackendRequiresProvider
	// BlockReasonProviderInactive 后端绑的供应商存在但未激活/缺 Key。
	BlockReasonProviderInactive = exec_target_svc.BlockReasonProviderInactive
	// BlockReasonRemoteProviderMissing 远端 agentred 未配置该供应商。
	BlockReasonRemoteProviderMissing = exec_target_svc.BlockReasonRemoteProviderMissing
