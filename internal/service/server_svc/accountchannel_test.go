...
// 把 send 里的二进制 Protobuf 帧逐条发出去，然后按 keepOpen 决定挂着还是断开。
...
// TestDialAccountChannel_DeliversSyncVersionSignals 通道的线上契约是 Protobuf
// notification；账号通道只消费 notification，不承担请求/响应。
//
...
func TestDialAccountChannel_DeliversSyncVersionSignals(t *testing.T) {
	Convey("账号级通道把 sync_version 信号交给同步引擎", t, func() {
		...
		srv, gotPath := accountChannelServer(t, "tok-9", prefix+"/v1/account/channel",
			[][]byte{{0x0a, 0x04, 0x0a, 0x02, 0x08, 0x2a}}, keepOpen)
