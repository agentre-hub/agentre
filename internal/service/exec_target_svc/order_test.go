...
func setupExecTargetOrderTest(t *testing.T) (context.Context, *execTargetOrderMocks, exec_target_svc.ExecTargetSvc) {
	t.Helper()
...
func TestPickExecTarget_GivenNoOverride_WhenSelfTargetInList_ThenSelfPromotedAndPicked(t *testing.T) {
	...
	m.remoteDevice.EXPECT().DeviceFingerprint().Return(devicefp.Carrier("sha256:me"), nil).AnyTimes()
	m.backend.EXPECT().ListByDevice(ctx, devicefp.Carrier("sha256:me")).Return([]*agent_backend_entity.AgentBackend{
