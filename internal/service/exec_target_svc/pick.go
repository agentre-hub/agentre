...
func (s *execTargetSvc) execTargetKind(ctx context.Context, be *agent_backend_entity.AgentBackend) string {
	if !BackendTargetsRemote(be) {
		return "local"
	}
...
func (s *execTargetSvc) accountDeviceFor(ctx context.Context, fingerprint devicefp.Carrier) (namedDeviceInfo, bool) {
...
func (s *execTargetSvc) evalRemoteDeviceAvailability(ctx context.Context, be *agent_backend_entity.AgentBackend) (BlockReason, string, error) {
...
//  3. 都查不到：未配对（R2b）。
func (s *execTargetSvc) evalNamedRemoteDeviceAvailability(ctx context.Context, be *agent_backend_entity.AgentBackend) (BlockReason, string, error) {
	if info, ok := s.accountDeviceFor(ctx, be.DeviceFingerprint); ok && info.kind == "desktop" {
		if !info.online {
...
// 查询的两个投影，不写两份取法。
func (s *execTargetSvc) evalExecTargetProjectPath(ctx context.Context, be *agent_backend_entity.AgentBackend, projectID int64) (BlockReason, string, error) {
