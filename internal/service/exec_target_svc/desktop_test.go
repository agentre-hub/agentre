...
// 有意义，其余方法返回零值（本包在桌面目标判定里只调 ListDevices）。
...
func (s stubServerSvc) DialDaemonRelay(context.Context, devicefp.Carrier, devicefp.Initiator) (client.ProtobufConnection, error) {
	return nil, nil
}
func (s stubServerSvc) DialDesktopRelay(context.Context, devicefp.Carrier, devicefp.Initiator) (client.ProtobufConnection, error) {
	return nil, nil
}
...
func setupDesktopTargetTest(t *testing.T, devices []server_svc.Device) (context.Context, *pickExecTargetMocks, exec_target_svc.ExecTargetSvc) {
	t.Helper()
	...
	m.remoteDevice.EXPECT().DeviceFingerprint().Return(devicefp.Carrier("sha256:self"), nil).AnyTimes()
	m.backend.EXPECT().ListByDevice(gomock.Any(), devicefp.Carrier("sha256:self")).Return(nil, nil).AnyTimes()
	...
	return context.Background(), m, exec_target_svc.NewExecTarget(nil)
}
...
func TestListExecTargetAvailability_GivenNamedDesktopAppNotRunning_ThenDesktopNotRunningReason(t *testing.T) {
	...
	assert.Equal(t, exec_target_svc.BlockReasonExecTargetDesktopNotRunning, statuses[0].Reason)
	assert.NotEqual(t, exec_target_svc.BlockReasonExecTargetOffline, statuses[0].Reason,
