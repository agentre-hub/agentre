...
func TestResolveSessionCwd_Exported(t *testing.T) {
	...
	cwd, err := exec_target_svc.ResolveSessionCwd(context.Background(),
...
func TestResolveSessionWorkspace(t *testing.T) {
	t.Run("本地会话 → deviceID 0 + CwdResolver 给出的 cwd", func(t *testing.T) {
		...
		deviceID, cwd, err := exec_target_svc.NewExecTarget(nil).ResolveSessionWorkspace(ctx, 5)
		require.NoError(t, err)
	...
	t.Run("会话已钉档 → 用钉住那一档的 backend，不是 Agent 主档", func(t *testing.T) {
		...
		deviceID, _, err := exec_target_svc.NewExecTarget(nil).ResolveSessionWorkspace(ctx, 6)
		require.NoError(t, err)
	...
	t.Run("会话不存在 → 报错", func(t *testing.T) {
		...
		_, _, err := exec_target_svc.NewExecTarget(nil).ResolveSessionWorkspace(ctx, 404)
		assert.Error(t, err)
	})
	...
	t.Run("sessionID 非法 → 不查库", func(t *testing.T) {
		...
		_, _, err := exec_target_svc.NewExecTarget(nil).ResolveSessionWorkspace(context.Background(), 0)
		assert.Error(t, err)
	})
	...
	t.Run("远端 backend 的 DeviceID 无法解析 → 报错而不是回落本机", func(t *testing.T) {
		...
		deviceID, _, err := exec_target_svc.NewExecTarget(nil).ResolveSessionWorkspace(ctx, 5)
		require.Error(t, err)
		assert.Equal(t, int64(0), deviceID)
	})
}
