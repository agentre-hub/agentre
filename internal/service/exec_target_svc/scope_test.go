...
func setupExecTargetTest(t *testing.T) (context.Context, *execTargetMocks, ExecTargetSvc) {
	t.Helper()
	...
	return context.Background(), m, NewExecTarget(nil)
}
...
func TestResolveLocalCommandScope_GivenSessionWithStickyExecTarget_ThenScopeFollowsPinnedBackendNotAgentPrimary(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, devicefp.Carrier("sha256:device-12"), scope.DeviceID, "范围必须跟着会话钉住的档(59/设备12),不是 Agent 当前主档(50/本机)")
}
...
func TestResolveLocalCommandScope_GivenExistingRemoteSession_WhenLocationChanges_ThenRereadsCurrentLocation(t *testing.T) {
	...
	gomock.InOrder(
		m.projectLocation.EXPECT().FindByProjectAndFingerprint(ctx, int64(42), devicefp.Carrier("sha256:device-9")).
...
func TestResolveLocalCommandScope_GivenPreSessionRemoteProject_WhenResolved_ThenUsesCurrentDeviceLocationWithoutCreatingSession(t *testing.T) {
	...
	m.projectLocation.EXPECT().FindByProjectAndFingerprint(ctx, int64(45), devicefp.Carrier("sha256:device-10")).
