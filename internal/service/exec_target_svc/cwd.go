...
//
// 引入回调而不是直接 import project_svc 是为了让本包不依赖 project_svc
...
// CurrentCwdResolver 是 RegisterCwdResolver 配对的读口（全仓 Register/accessor 约定）：
// 跨包单测据此保存并还原这个进程级注入点，而不是一律还原成 nil。
func CurrentCwdResolver() CwdResolver { return resolveCwdFn }
...
// 注入)。放在 exec_target_svc 是因为这里本来就持有 session → agent → backend 的查询
...
func (s *execTargetSvc) ResolveSessionWorkspace(ctx context.Context, sessionID int64) (int64, string, error) {
	if sessionID <= 0 {
