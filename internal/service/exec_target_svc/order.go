...
// 提前不触发（浏览器语境正是这种情况）。
func (s *execTargetSvc) selfFingerprint(ctx context.Context) devicefp.Carrier {
...
// BackendTargetsRemote 报告一个 backend 是否要派到「别的机器」上跑。判据本身住在
...
func BackendTargetsRemote(be *agent_backend_entity.AgentBackend) bool {
...
func (s *execTargetSvc) selfBackendIDs(ctx context.Context, targets []*agent_entity.AgentExecTarget) ([]int64, error) {
	...
	if err != nil {
		return nil, operationFailedWithCause(ctx, err, zap.String("deviceId", string(fp)))
	}
