...
func (d directDialer) DialDesktopRelay(_ context.Context, desktopFingerprint, peerFingerprint string) (client.ProtobufConnection, error) {
	...
	_, err = protorpc.CallMethod(ctx, conn.Conn(), uint32(agentrewire.RpcMethod_RPC_METHOD_AUTH_ACCOUNT), &agentrewire.AuthAccountRequest{Credential: "test", ProtocolVersion: wireversion.Protocol, MinSupportedProtocolVersion: wireversion.MinSupported}, func() *agentrewire.AuthAccountResponse { return &agentrewire.AuthAccountResponse{} })
	if err != nil {
		_ = conn.Close()
		return nil, err
	}
...
func TestPeerSvc_GivenRemoteDesktopSessions_WhenList_ThenReturnFullSummaries(t *testing.T) {
	url := fakePeerServer(t, peer.ProtobufInboundDeps{ListSessions: func(context.Context, wire.SessionListParams) (*wire.SessionListResult, error) {
	...
	result, err := svc.ListSessions(context.Background(), peer_svc.ListSessionsRequest{Fingerprint: "sha256:peer-desktop"})
	require.NoError(t, err)
...
func TestPeerSvc_GivenTargetDesktopAppNotRunning_WhenList_ThenDesktopSentinelSurfaces(t *testing.T) {
	...
	_, err := svc.ListSessions(context.Background(), peer_svc.ListSessionsRequest{Fingerprint: "sha256:peer-desktop"})
	require.Error(t, err)
